...
bool CUser::SynchroniseWithProfile( bool bForceUpdate )
{
	...
	CheckAndSyncAutoGerneratedSiteSuffix();
...
/*********************************************************************************
	...
	void CUser::CheckForExistingUDNGifSiteSuffixIsNull()

	Author:		Steve Francis
	Created:	08/04/2010
	Purpose:	Checks if the site suffix is null and if we're on a site that uses AutoGen names	
				Checks if one exists and goes and gets the users autogen nickname from the signin system

*********************************************************************************/
void CUser::CheckForExistingUDNGifSiteSuffixIsNull()
{
	m_InputContext.WriteInputLog("USER", "CheckForExistingUDNGifSiteSuffixIsNull");
...
/*********************************************************************************
	...
	void CUser::CheckAndSyncAutoGerneratedSiteSuffix()
...
void CUser::CheckAndSyncAutoGerneratedSiteSuffix()
{
	m_InputContext.WriteInputLog("USER", "CheckAndSyncAutoGerneratedSiteSuffix");
	...
	else
	{
		...
		if (bSuccess)
		{
			...
			if (m_syncByDefault && pProfile != NULL && pProfile->AttributeExistsForService("lastupdatedcpp"))
			{
				...
				{
					//Just check for the SiteSuffix being Null, if on a kids site that needs UDNG and there is already one
					// in identity
					m_InputContext.WriteInputLog("USER", "Check for UDNG name from existing site");
					CheckForExistingUDNGifSiteSuffixIsNull();
				}
			}
		}
