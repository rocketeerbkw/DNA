// SubmitSubbedEntryPageBuilder.cpp: implementation of the CSubmitSubbedEntryPageBuilder class.
//
//////////////////////////////////////////////////////////////////////
...
/*********************************************************************************
	...
	CSubmitSubbedEntryPageBuilder::CSubmitSubbedEntryPageBuilder(CInputContext& inputContext)
																			 ,
	Author:		Kim Harries
	Created:	15/12/2000
	...
	Outputs:	-
	Returns:	-
	Purpose:	Constructs the minimal requirements for a CSubmitSubbedEntryPageBuilder object.
...
/*********************************************************************************
	...
	CSubmitSubbedEntryPageBuilder::~CSubmitSubbedEntryPageBuilder()
...
/*********************************************************************************
	...
	CWholePage* CSubmitSubbedEntryPageBuilder::Build()
	...
	Purpose:	Constructs the XML for the page that allows subs to submit their
				articles once they are finished with subbing them.

*********************************************************************************/

bool CSubmitSubbedEntryPageBuilder::Build(CWholePage* pWholePage)
{
	...
	CSubmitSubbedEntryForm	Form(m_InputContext);

	// get the viewing user
	pViewer = m_InputContext.GetCurrentUser();
	// initiliase the whole page object and set the page type
	bSuccess = InitPage(pWholePage, "SUBMIT-SUBBED-ENTRY",true);
	// do an error page if non-registered user, or a user who is neither
	// a sub nor an editor
	// otherwise proceed with the recommendation page
	if (pViewer == NULL || !pViewer->GetIsEditor() && !pViewer->GetIsSub())
	{
		bSuccess = bSuccess && pWholePage->SetPageType("ERROR");
		bSuccess = bSuccess && pWholePage->AddInside("H2G2", "<ERROR TYPE='NOT-SUBEDITOR'>You cannot return an Entry as subbed unless you are registered and either an Editor or a Sub Editor.</ERROR>");
	}
	else
	{
		...
		if (sCommand.CompareText("submit"))
		{
			...
			bSuccess = bSuccess && Form.SubmitSubbedEntry(pViewer, ih2g2ID, sComments);
		}
		else
		{
			...
			if (ih2g2ID > 0)
			{
				...
				// is not a valid one
				bSuccess = bSuccess && Form.CreateFromh2g2ID(pViewer, ih2g2ID);
			}
			else
			{
	...
	TDVASSERT(bSuccess, "CSubmitSubbedEntryPageBuilder::Build() failed");
	return bSuccess;
}
