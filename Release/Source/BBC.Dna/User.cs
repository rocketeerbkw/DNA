...
namespace BBC.Dna
{
    ...
    public class User : DnaInputComponent, IUser
    {
        ...
        /// <summary>
        /// Checks if the site suffix is null and if we're on a site that uses AutoGen names	
		///		Checks if one exists and goes and gets the users autogen nickname from the signin system
        /// </summary>
        private void CheckForExistingUDNGifSiteSuffixIsNull()
        {
            InputContext.Diagnostics.WriteToLog("User", "CheckForExistingUDNGifSiteSuffixIsNull");

	        if (InputContext.GetSiteOptionValueInt("User","UseSiteSuffix") > 0 && InputContext.GetSiteOptionValueString("User","AutoGeneratedNames").Length > 0)
	        {
		        if (InputContext.GetSiteOptionValueBool("General","IsKidsSite"))
		        {
                    if (_siteSuffix.Length == 0)
			        {
                        CheckAutoGeneratedName();
			        }
		        }
