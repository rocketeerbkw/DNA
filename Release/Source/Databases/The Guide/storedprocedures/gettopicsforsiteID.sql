CREATE PROCEDURE gettopicsforsiteid @isiteid INT, @itopicstatus INT, @includearchived INT
AS
BEGIN
	...
	SELECT tp.TopicID, ge.h2g2ID, ge.SiteID, tp.TopicStatus, ge.Subject AS "TITLE" , ge.ForumID, 
				...
				"FP_ElementID" = 					
					CASE 
						WHEN fp.ElementStatus IN (@iTopicStatus,@TopicStatus2) THEN te.TopicElementID
						ELSE 0 
					END, 				
				"FP_ElementStatus" =
					CASE 
						WHEN fp.ElementStatus IN (@iTopicStatus,@TopicStatus2) THEN fp.ElementStatus 
						ELSE NULL 
					END, 					
				"FP_Template" =
					CASE 
						WHEN fp.ElementStatus IN (@iTopicStatus,@TopicStatus2) THEN fp.templatetype 
						ELSE NULL 
					END, 
				"FP_Position" =
					CASE 
						WHEN fp.ElementStatus IN (@iTopicStatus,@TopicStatus2) THEN fp.frontpageposition 
						ELSE NULL 
					END, 
				"FP_Title" =
					CASE 
						WHEN fp.ElementStatus IN (@iTopicStatus,@TopicStatus2) THEN fp.title 
						ELSE NULL 
					END,
