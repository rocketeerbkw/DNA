/*********************************************************************************
	...
	create procedure getusersarticleswithaudioassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null as
	...
	Purpose:	Get the details of articles with Audio assets for a given user
	
*********************************************************************************/
CREATE PROCEDURE getusersarticleswithaudioassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null
AS
BEGIN	
...
IF @owner IS NOT NULL
	BEGIN
		--Get total articles with audio assets for the viewing user.
		SELECT @total =  COUNT(*) FROM dbo.AudioAsset A WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = A.MediaAssetID
...
ELSE
	BEGIN
		--Get total articles with audio assets that have passed moderation for the user given.
		SELECT @total =  COUNT(*) FROM dbo.AudioAsset A WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = A.MediaAssetID
				INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON MA.ID = AMA.MediaAssetID
...
IF @owner IS NOT NULL
	...
	IF @sortbycaption IS NOT NULL
		...
		INSERT INTO #tempusrassets (MediaAssetID)
			...
			FROM dbo.AudioAsset A WITH(NOLOCK) 
			INNER JOIN dbo.MediaAsset AS MA WITH(NOLOCK) ON MA.ID = A.MediaAssetID
			INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON MA.ID = AMA.MediaAssetID
...
INNER JOIN dbo.AudioAsset A WITH(NOLOCK) ON A.MediaAssetID = tmp.MediaAssetID
