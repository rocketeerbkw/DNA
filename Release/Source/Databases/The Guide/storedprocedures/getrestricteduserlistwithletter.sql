/*********************************************************************************
	...
	create procedure getrestricteduserlistwithletter	@viewinguserid int, 
														@skip int, 
														@show int, 
														@usertypes int, 
														@siteid int, 
	...
	Inputs:		@viewinguserid int	- the viewer looking at the list, 
				...
				@letter varchar(1)  - beginning letter of their email to restrict to
	Outputs:	
	Returns:	Restricted Users dataset
	Purpose:	Gets the restricted users requested with a given beginning letter
*********************************************************************************/
CREATE PROCEDURE getrestricteduserlistwithletter @viewinguserid int, @skip int, @show int, @usertypes int, @siteid int, @letter varchar(1)
AS
	...
	SELECT @sql = N'
			...
			(	
				SELECT (ROW_NUMBER() OVER (ORDER BY u.Email) - 1) AS ''Row'', 
					u.UserID,
					u.UserName, 
					u.FirstNames, 
					u.LastName, 
				...
				WHERE u.EMail LIKE @letter + ''%'' '

	IF (@siteid <> 0)
	BEGIN
		SELECT @sql = @sql + N' AND p.SiteID = @siteid '
	END
	
	IF (@usertypes <> 0)
	BEGIN
		SELECT @sql = @sql + N' AND p.PrefStatus = @usertypes '
	END
	ELSE
	BEGIN
		SELECT @sql = @sql + N' AND (p.PrefStatus = 1 OR p.PrefStatus = 4) '
	END
