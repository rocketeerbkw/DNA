/*********************************************************************************
	...
	create procedure getusersvideoassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null as
	...
	Purpose:	Get the details of video assets for a given user
	
*********************************************************************************/
CREATE PROCEDURE getusersvideoassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null
AS
BEGIN	
...
IF @owner IS NOT NULL
	BEGIN
		--Get total video assets for user.
		SELECT @total =  COUNT(*) FROM MediaAsset MA WITH(NOLOCK) 
			INNER JOIN dbo.VideoAsset AS V WITH(NOLOCK) ON V.MediaAssetID = MA.ID
			WHERE MA.OwnerID = @userid AND (MA.Hidden <> 1 OR MA.Hidden IS NULL)			
	END
ELSE
	BEGIN
		--Get total video assets that have passed moderation for the given user.
		SELECT @total =  COUNT(*) FROM MediaAsset MA WITH(NOLOCK) 
			INNER JOIN dbo.VideoAsset AS V WITH(NOLOCK) ON V.MediaAssetID = MA.ID
...
CREATE TABLE #tempusrvideoassets(id int NOT NULL IDENTITY(0,1)PRIMARY KEY, MediaAssetID int NOT NULL)
...
IF @owner IS NOT NULL
	...
	BEGIN
		INSERT INTO #tempusrvideoassets (MediaAssetID)
...
ELSE
	...
	DELETE FROM #tempusrvideoassets where id > @lastindex or id < @firstindex

--Get count.
select @count = count(*) from ( select * from #tempusrvideoassets) as s
...
FROM #tempusrvideoassets tmp
INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = tmp.MediaAssetID
INNER JOIN dbo.VideoAsset AS V WITH(NOLOCK) ON V.MediaAssetID = tmp.MediaAssetID
