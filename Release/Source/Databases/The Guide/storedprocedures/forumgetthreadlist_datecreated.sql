create procedure forumgetthreadlist_datecreated		@forumid int, 
													@firstindex int, 
													@lastindex int, 
													@threadcount int, 
													@forumpostcount int,
...
as
			...
			SELECT	t.ThreadID, 
				...
				FROM (select ThreadID , rownumber id from (select ThreadID, ROW_NUMBER() over (order by DateCreated desc) as rownumber FROM Threads WITH(NOLOCK) where ForumID = @forumid AND VisibleTo IS NULL) t where rownumber between @firstindex+1 and @lastindex+1) tt
