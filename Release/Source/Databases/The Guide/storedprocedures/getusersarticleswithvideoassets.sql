/*********************************************************************************
	...
	create procedure getusersarticleswithvideoassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null as
	...
	Outputs:	Returns the Users Video Assets
	Purpose:	Get the details of articles with Video assets for a given user
	
*********************************************************************************/
CREATE PROCEDURE getusersarticleswithvideoassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null
AS
BEGIN	
...
IF @owner IS NOT NULL
	BEGIN
		--Get total articles with video assets for the viewing user.
		SELECT @total =  COUNT(*) FROM dbo.VideoAsset V WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = V.MediaAssetID
				INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON MA.ID = AMA.MediaAssetID
				WHERE MA.OwnerID = @userid AND (MA.Hidden <> 1 OR MA.Hidden IS NULL) 			
	END
ELSE
	BEGIN
		--Get total articles with video assets that have passed moderation for the user given.
		SELECT @total =  COUNT(*) FROM dbo.VideoAsset V WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = V.MediaAssetID
				INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON MA.ID = AMA.MediaAssetID
...
--if it is the viewing user select only the video assets that have not been failed else only the passed ones
IF @owner IS NOT NULL
	BEGIN
	...
	BEGIN
		INSERT INTO #tempusrassets (MediaAssetID)
			...
			FROM dbo.VideoAsset V WITH(NOLOCK) 
			INNER JOIN dbo.MediaAsset AS MA WITH(NOLOCK) ON MA.ID = V.MediaAssetID
			INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON MA.ID = AMA.MediaAssetID
			LEFT JOIN dbo.PageVotes pv WITH(NOLOCK) ON AMA.entryid=pv.itemid/10 and pv.itemtype=1
			LEFT JOIN dbo.Votes vote WITH(NOLOCK) ON pv.voteid=vote.voteid and vote.type=3
...
INNER JOIN dbo.VideoAsset V WITH(NOLOCK) ON V.MediaAssetID = tmp.MediaAssetID
INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = tmp.MediaAssetID
INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON AMA.MediaAssetID = tmp.MediaAssetID
