create procedure forumgetthreadlist_lastposted_includingstickythreads		@forumid int, 
													@firstindex int, 
													@lastindex int, 
													@threadcount int, 
													@forumpostcount int,
...
as
			...
			SELECT	t.ThreadID, 
				...
				FROM (
					select ThreadID , rownumber id from 
						(
							select tr.ThreadID, ROW_NUMBER() over (order by sst.sortby desc, LastPosted desc) as rownumber 
