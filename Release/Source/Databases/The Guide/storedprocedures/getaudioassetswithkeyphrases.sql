/*********************************************************************************
	...
	create procedure getaudioassetswithkeyphrases @keyphraselist VARCHAR(8000),
	...
	Outputs:	Audio Asset dataset
	Purpose:	Returns the Audio Asset dataset with the given key phrases
	
*********************************************************************************/
CREATE PROCEDURE getaudioassetswithkeyphrases @keyphraselist VARCHAR(8000), @siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null
AS
BEGIN	
...
--Get total audio assets for site with key phrases.
declare @total int
SELECT @total =  COUNT(*) FROM (SELECT MA.ID FROM udf_splitvarchar(@keyphraselist) as s
	...
	INNER JOIN dbo.AudioAsset aud WITH (NOLOCK) ON aud.MediaAssetID = MA.ID
