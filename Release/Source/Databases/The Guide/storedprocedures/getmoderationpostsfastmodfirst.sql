...
*/
CREATE PROCEDURE getmoderationpostsfastmodfirst @userid int, @status int = 0, @alerts int = 0,  @lockeditems int = 0, @fastmod int = 0, @issuperuser bit = 0, @modclassid int = NULL, @postid int = NULL, @duplicatecomplaints bit = 0, @show int = 10
As
...
WHERE tm.NewPost=1 AND tm.Status = @status 
	...
	AND (@fastmod = 0 or (CASE WHEN fmf.ForumID IS NOT NULL THEN 1 ELSE 0 END) = @fastmod)
ORDER BY userlocked DESC, isnull(fmf.ForumID, 1) desc, tm.DateQueued asc
