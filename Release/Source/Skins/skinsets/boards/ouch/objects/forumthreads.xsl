    ...
    <xsl:template match="FORUMTHREADS[THREAD]" mode="object_forumthreads">
        <xsl:if test="$siteClosed = 'false'">
            <p class="dna-boards-startanewdiscussion">
            	<a href="/ouch/messageboards/AddThread?forum={@FORUMID}&amp;article={/H2G2/FORUMSOURCE/ARTICLE/ARTICLEINFO/H2G2ID}" class="id-cta">
                    <xsl:call-template name="library_memberservice_require">
                        <xsl:with-param name="ptrt">
                            <xsl:text>/ouch/messageboards/AddThread?forum=</xsl:text>
        ...
        <h2 class="offscreen">Start of discussions</h2>
