...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:template name="USERCOMPLAINT_JAVASCRIPT">
		...
		function checkUserComplaintForm(){
			...
			if (text == '' || text == '</xsl:text>
			<xsl:value-of select="$m_ucdefaultcomplainttext"/>
			<xsl:text disable-output-escaping="yes">'){
				alert('</xsl:text>
			<xsl:value-of select="$m_ucnodetailsalert"/>
			<xsl:text disable-output-escaping="yes">');
				return false;
			}
			...
			if (!CheckEmailFormat(document.UserComplaintForm.EmailAddress.value)){
				alert('</xsl:text>
			<xsl:value-of select="$m_ucinvalidemailformat"/>
			<xsl:text disable-output-escaping="yes">');
	...
	<xsl:attribute-set name="mUSER-COMPLAINT-FORM_t_textarea">
		...
		<xsl:attribute name="class">txtArea</xsl:attribute>
