...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:variable name="articlesearchbase">
		...
		<xsl:text>?contenttype=-1&amp;phrase=_memory&amp;show=8</xsl:text>
	</xsl:variable>
	...
	<xsl:variable name="check_client">
		<xsl:choose>
	...
	<xsl:variable name="client">
		<xsl:choose>
			<xsl:when test="msxsl:node-set($clients)/list/item[client=$check_client]">
				<xsl:value-of select="$check_client"/>
			</xsl:when>
			<xsl:otherwise><xsl:value-of select="$vanilla_client"/></xsl:otherwise>
		</xsl:choose>		
	</xsl:variable>
