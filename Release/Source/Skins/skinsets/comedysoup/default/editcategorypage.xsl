...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:with-param name="message">EDITCATEGORY_MAINBODY</xsl:with-param>
	<xsl:with-param name="pagename">editcategorypage.xsl</xsl:with-param>
	</xsl:call-template>
	<!-- DEBUG -->
	...
	<xsl:template match="EDITCATEGORY" mode="r_editcat">
		<font xsl:use-attribute-sets="mainfont">
			...
			<br/>
			<xsl:apply-templates select="HIERARCHYDETAILS" mode="c_editcat"/>
			<br/>
	...
	<xsl:template match="DESCRIPTION" mode="r_editcat">
		<xsl:apply-templates/>
		<xsl:text> </xsl:text>
		<xsl:apply-templates select="." mode="t_editcat"/>
		<br/>
	</xsl:template>
	<!--
	...
	Use: Presentation of an individual subject member node
	 -->
	<xsl:template match="SUBJECTMEMBER" mode="r_editcat">
		<xsl:apply-templates select="NAME" mode="t_nodename_ec"/>
		...
		<xsl:apply-templates select="." mode="c_deletesubject"/>
	</xsl:template>
	<!--
	...
	Use: 'Add an article to this subject' functionality
	 -->
	<xsl:template match="HIERARCHYDETAILS" mode="r_addarticle">
		<xsl:copy-of select="$m_addarticletosubject"/>
		<xsl:apply-templates select="." mode="t_addarticlednaid"/>
		<br/>
		<xsl:apply-templates select="." mode="t_addarticlesubmit"/>
	</xsl:template>
		<!--
