...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<!-- <xsl:variable name="username">
		<xsl:choose>
			<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME = 'UseSiteSuffix']/VALUE = '1'">
				<xsl:choose>
					<xsl:when test="not(/H2G2/VIEWING-USER/USER/SITESUFFIX)">
						<xsl:value-of select="concat('*', /H2G2/VIEWING-USER/USER/USERNAME)" />					
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="/H2G2/VIEWING-USER/USER/SITESUFFIX" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:variable name="id_settingslink">
		<xsl:value-of select="concat($idURL, 'users/dash?', $id_params, $id_kids)" />		
	</xsl:variable>
	
	<xsl:variable name="id_morelink">
		<xsl:value-of select="concat($idURL, 'users/dash/more?', $id_params, $id_kids)" />
	</xsl:variable>	
		          ...
		          <xsl:otherwise>
			          ...
			          <xsl:choose>
			          	...
			          	<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME = 'UseSiteSuffix']/VALUE = '1' and not(/H2G2/VIEWING-USER/USER/SITESUFFIX) and $sitesuffixoptional = 'false'">
			          		...
			          		<p>
			          			<xsl:text>No site suffix! </xsl:text>
			          			<a>
			          				<xsl:attribute name="href">
				          				<xsl:choose>
				          					<xsl:when test="$autogenname_required = 'true'"> 
