...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:variable name="sitesuffixoptional">
		<xsl:value-of select="boolean(/H2G2/SITE/@ID = '1')" />
	</xsl:variable>

	<xsl:variable name="id_autogennameurl">
		<!-- 'target_resource=', $policy_encoded, '&amp;ptrt=', $id_ptrt, $root_encoded, $referrer -->
		<xsl:value-of select="concat(/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME='AutoGeneratedNames']/VALUE, '?onwardto=', $id_ptrt, $root_encoded, $referrer)" />
	</xsl:variable>
	
	<xsl:variable name="autogenname_required">
		<xsl:value-of select="boolean(/H2G2/SITE/SITEOPTIONS/SITEOPTION[@GLOBAL='0'][NAME = 'AutoGeneratedNames'] and not(/H2G2/VIEWING-USER/USER/SITESUFFIX))" />
		<!-- <xsl:value-of select="boolean(/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME = 'AutoGeneratedNames']/VALUE and not(/H2G2/VIEWING-USER/USER/SITESUFFIX))" /> -->
	</xsl:variable>
	
	<xsl:variable name="sitesuffix_required">
		<xsl:value-of select="boolean(/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME = 'UseSiteSuffix']/VALUE = '1')" />
	</xsl:variable>	
		...
		<xsl:choose>
			...
			<xsl:when test="$serverenvironment = 'stage'">
				<xsl:choose>
					<xsl:when test="$hosttype = 'https'">
						<xsl:value-of select="$hosttype"/>
						<xsl:text>%3A%2F%ssl.stage.bbc.co.uk</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$hosttype"/>
	...
	<xsl:variable name="id_loginpath">
		<xsl:choose>
			<xsl:when test="not(/H2G2/VIEWING-USER/IDENTITY)">
				<xsl:choose>
					<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME='UseIDV4']/VALUE = '1'"><xsl:text>/signin?</xsl:text></xsl:when>
					<xsl:otherwise><xsl:text>/users/login?</xsl:text></xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
	...
	<xsl:variable name="id_registerlink">
		<xsl:choose>
			<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME='UseIDV4']/VALUE = '1'">
				<xsl:value-of select="concat($idURL, '/register?', $id_params)" />
			</xsl:when>
			<xsl:otherwise>
        <xsl:choose>
          <xsl:when test="$autogenname_required = 'true'">
            <xsl:value-of select="concat($idURL, 'users/register?', 'target_resource=', $policy_encoded, '&amp;ptrt=', $id_autogennameurl)" />
          </xsl:when>
          <xsl:otherwise>
	...
	<xsl:variable name="id_settingslink">
		<xsl:choose>
			...
			<xsl:otherwise><xsl:value-of select="concat($idURL, '/users/dash?', $id_params, $id_kids)" /></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="id_signoutlink">
		<xsl:choose>
			<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME='UseIDV4']/VALUE = '1'">
				<xsl:value-of select="concat($idURL, '/signout?', $id_params)" />
			</xsl:when>
			<xsl:otherwise><xsl:value-of select="concat($idURL, '/users/logout?', $id_params)" /></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="sso_nopostsigninlink">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select="concat($id_signinlink, $id_kids)" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="registerlink">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select="concat($id_registerlink, $id_kids)" />				
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>	
			...
			<xsl:when test="/H2G2/VIEWING-USER/USER">
				<xsl:choose>
					<xsl:when test="/H2G2[@TYPE='MULTIPOSTS' or @TYPE='USERPAGE' or @TYPE='MESSAGEFRAME' or @TYPE='JOURNAL']">
						<xsl:choose>
							<xsl:when test="$autogenname_required = 'true'">
								<xsl:value-of select="$id_autogennameurl" />
							</xsl:when>	
							<xsl:otherwise>
								<xsl:value-of select="concat($root, 'AddThread?inreplyto=', @POSTID)"/>
							</xsl:otherwise>						
				...
				<xsl:choose>
					<xsl:when test="/H2G2[@TYPE='THREADS']">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:value-of select="concat($idURL, $id_loginpath, 'target_resource=', $policy_encoded, '&amp;ptrt=', $id_ptrt, $root_encoded, 'AddThread?forum=', /H2G2/FORUMTHREADS/@FORUMID, '&amp;article=', /H2G2/FORUMSOURCE/ARTICLE/ARTICLEINFO/H2G2I, $id_kids)" />
							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:when test="(/H2G2[@TYPE='ARTICLE']) and (/H2G2/ARTICLEFORUM/FORUMTHREADPOSTS[@GUESTBOOK=1]/POST)">
							...
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="/H2G2/FORUMSOURCE">
										...
										<xsl:value-of select="concat($idURL, $id_loginpath, 'target_resource=', $policy_encoded, /H2G2/FORUMSOURCE/ARTICLE/ARTICLEINFO/H2G2ID, $id_kids)" />
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="concat($idURL, $id_loginpath, 'target_resource=', $policy_encoded,'&amp;ptrt=', $id_ptrt, $root_encoded, 'AddThread%3Fforum=', /H2G2/ARTICLEFORUM/FORUMTHREADS/@FORUMID, '%26article=', /H2G2/ARTICLE/ARTICLEINFO/H2G2ID, $id_kids)" />
									</xsl:otherwise>
								</xsl:choose>
							</xsl:otherwise>
						</xsl:choose>
					...
					<xsl:otherwise>
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:value-of select="concat($idURL, $id_loginpath, 'target_resource=', $policy_encoded,'&amp;ptrt=', $id_ptrt, $root_encoded, 'AddThread%3Fpa=postforum%26pt=forumtype%26forumtype=art%26pt=forum%26forum=', /H2G2/ARTICLEFORUM/FORUMTHREADPOSTS/@FORUMID, '%26pt=article%26article=', /H2G2/ARTICLE/ARTICLEINFO/H2G2ID, $id_kids)" />								
							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="concat($id_signinlink, 'useredit', $id_kids)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
		          ...
		          <xsl:otherwise>
			          ...
			          <xsl:choose>
			          	...
			          	<xsl:when test="/H2G2/SITE/SITEOPTIONS/SITEOPTION[NAME = 'UseSiteSuffix']/VALUE = '1' and not(/H2G2/VIEWING-USER/USER/SITESUFFIX) and $sitesuffixoptional = 'false'">
			          		<!-- <meta http-equiv="refresh" content="0;url={$id_autogennameurl}"/> -->
			          		<p>
							...
							<ul>
								<li>
									<xsl:apply-templates select="/H2G2/VIEWING-USER/USER" mode="username">
										<xsl:with-param name="stringlimit" select="$idstringlimit" />									
									</xsl:apply-templates>									
								</li>  
								<li>|</li>
								<li><a>
									<xsl:attribute name="href">
										<xsl:value-of select="concat($id_settingslink, id_kids)" />									
									</xsl:attribute>
									Settings </a></li>  
								<li>|</li>
