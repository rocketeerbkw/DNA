...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
        ...
        <li>
          ...
          <a href="#">
            <xsl:attribute name="href">
              moderateposts?modclassid=<xsl:value-of select="@CLASSID"/>&amp;s_classview=<xsl:value-of select="@CLASSID"/>&amp;alerts=<xsl:choose>
                <xsl:when test="/H2G2/POSTMODERATION/@ALERTS = 1">1</xsl:when>
                <xsl:otherwise>0</xsl:otherwise>
              </xsl:choose>&amp;referrals=<xsl:choose>
                <xsl:when test="/H2G2/POSTMODERATION/@REFERRALS = 1">1</xsl:when>
                <xsl:otherwise>0</xsl:otherwise>
              </xsl:choose>&amp;locked=<xsl:choose>
                <xsl:when test="/H2G2/POSTMODERATION/@LOCKEDITEMS = 1">1</xsl:when>
                <xsl:otherwise>0</xsl:otherwise>
              </xsl:choose>
            </xsl:attribute>
  ...
  <xsl:template match="POST" mode="post_list">
    ...
    <xsl:if test="not(/H2G2/POSTMODERATION/POST[1]/@POSTID = /H2G2/POSTMODERATION/POST[2]/@POSTID) or /H2G2/POSTMODERATION/POST[1]/@POSTID = 0">
      <div class="tools">
        ...
        <span class="clear"/>
        <xsl:if test="$test_IsEditor and POSTSTYLE != 4">
  ...
  <xsl:template match="POST[POSTSTYLE=4]" mode="user_info">
    <p class="postUserBar">
      <xsl:choose>
        <xsl:when test="$test_IsEditor">
          <!--<xsl:apply-templates select="STATUS" mode="user_status"/>-->
  ...
  <xsl:template match="POST[POSTSTYLE=4]" mode="post_info">
    <xsl:variable name="priorityPost">
      <xsl:choose>
        <xsl:when test="@ISPRIORITYPOST =1">
          <xsl:text>** High Priority ** </xsl:text>
    ...
    <div class="postContent">
      <table>
        <tr>
          <xsl:if test="POSTSTYLE=4">
            <td>
              <a href="http://twitter.com/{USER/USERNAME}" target="_blank">
                <img src="{USER/STATUS/@SITESUFFIX}" />
              </a>
            </td>
          </xsl:if>
          <td>
