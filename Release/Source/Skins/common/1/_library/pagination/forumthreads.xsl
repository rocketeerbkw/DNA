    ...
    <xsl:template match="FORUMTHREADS" mode="library_pagination_forumthreads">
        <ul class="pagination">
            ...
            <li class="previous">
                <xsl:choose>
                    <xsl:when test="@SKIPTO > 0">
                        <a href="{$root}/F{@FORUMID}?skip={@SKIPTO - @COUNT}">
                            <span class="arrow">
            ...
            <li class="last">
              <xsl:choose>
                    <xsl:when test="@MORE != 0">
                    	<a href="{$root}/F{@FORUMID}?skip={(floor(@TOTALTHREADS div @COUNT) * @COUNT)}">
    ...
    <xsl:template match="FORUMTHREADS" mode="library_pagination_pagelist">
        ...
        <xsl:if test="($totalPages > 1) and ($counter > ($currentPage - 6) ) and ($counter &lt; ($currentPage + 6) )">
            <li>
                ...
                <a href="{$root}/F{@FORUMID}?skip={@COUNT * ($counter - 1)}">
