...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
    ...
    <xsl:template match="VIEWING-USER[USER or SIGNINNAME]" mode="library_sso_settingsurl">
        <xsl:value-of select="$root"/>
        <xsl:text>/userdetails</xsl:text>
    </xsl:template>
    
    <xsl:template match="VIEWING-USER" mode="library_sso_settingsurl">
    </xsl:template>
