...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
		...
		<xsl:template match="USEREVENTLIST" mode="library_pagination_forumthreadposts">
			<xsl:variable name="queryuser">
        <xsl:value-of select="concat('&amp;s_user=', @USERID)"/>
			</xsl:variable>
			...
			<xsl:variable name="querysite">
				<xsl:if test="/H2G2/PARAMS/PARAM[NAME = 's_siteid']/VALUE != '0'">
					<xsl:value-of select="concat('&amp;s_siteid=', /H2G2/PARAMS/PARAM[NAME = 's_siteid']/VALUE)"/>
				</xsl:if>
			</xsl:variable>
      <xsl:variable name="modclasssite">
        <xsl:if test="/H2G2/PARAMS/PARAM[NAME = 's_modclassid']/VALUE != '0'">
          <xsl:value-of select="concat('&amp;s_modclassid=', /H2G2/PARAMS/PARAM[NAME = 's_modclassid']/VALUE)"/>
        </xsl:if>
      </xsl:variable>
			
			<xsl:variable name="querystring">
				<xsl:value-of select="concat($queryuser,$querystart, $queryend, $querysite, $modclasssite)"/>
			</xsl:variable>
			
			<xsl:variable name="itemcount" select="count(USEREVENTLIST/USEREVENT)" />
			...
			<div class="dna-fr">
				<ul class="pagination">
					<li class="first dna-button">
						<xsl:choose>
							<xsl:when test="@STARTINDEX > 0">
								<a href="{$root}/hostdashboarduseractivity?s_startindex=0{$querystring}">
					...
					<li class="previous dna-button">
						<xsl:choose>
							<xsl:when test="@STARTINDEX > 0">
								<a href="{$root}/hostdashboarduseractivity?s_startindex={@STARTINDEX - $itemcount}{$querystring}">
									<span class="arrow">
						...
						<xsl:choose>
							...
							<xsl:choose>
								<xsl:when test="@TOTALITEMS mod $itemcount =0">
									<a href="{$root}/hostdashboarduseractivity?s_startindex={(floor(@TOTALITEMS div $itemcount)-1) * $itemcount}{$querystring}">
										<xsl:text> Last</xsl:text>
									</a>
								</xsl:when>
								<xsl:otherwise>
									<a href="{$root}/hostdashboarduseractivity?s_startindex={(floor(@TOTALITEMS div $itemcount) * $itemcount)}{$querystring}">
										<xsl:text> Last</xsl:text>
									</a>
								</xsl:otherwise>
							</xsl:choose>
		...
		<xsl:template match="USEREVENTLIST" mode="library_pagination_pagelist">
			<xsl:param name="querystring" />
			<xsl:param name="itemcount" select="count(USEREVENTLIST/USEREVENT)" />
			...
			<xsl:if test="($totalPages > 1) and ($counter > ($currentPage - 10) ) and ($counter &lt; ($currentPage + 10) )">
				<li>
					...
					<a href="{$root}/hostdashboarduseractivity?s_startindex={$itemcount * ($counter - 1)}{$querystring}">
