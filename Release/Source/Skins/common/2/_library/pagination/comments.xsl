				...
				<div class="dna-fr">
					<ul class="pagination">
						<li class="first dna-button">
								<xsl:choose>
											<xsl:when test="STARTINDEX > 0">
													<a href="{$root}/commentslist?s_forumid={$forumid}&amp;s_startindex=0{$querystring}">
							...
							<li class="previous dna-button">
									<xsl:choose>
											<xsl:when test="STARTINDEX > 0">
													<a href="{$root}/commentslist?s_forumid={$forumid}&amp;s_startindex={@STARTINDEX - $itemcount}{$querystring}">
															<span class="arrow">
											...
											<xsl:when test="TOTALCOUNT > STARTINDEX + $itemcount">
												<xsl:choose>
													<xsl:when test="TOTALCOUNT mod $itemcount =0">
														<a href="{$root}/commentslist?s_forumid={$forumid}&amp;s_startindex={(floor(TOTALCOUNT div $itemcount)-1) * $itemcount}{$querystring}">
															<xsl:text> Last</xsl:text>
														</a>
													</xsl:when>
													<xsl:otherwise>
														<a href="{$root}/commentslist?s_forumid={$forumid}&amp;s_startindex={(floor(TOTALCOUNT div $itemcount) * $itemcount)}{$querystring}">
															<xsl:text> Last</xsl:text>
														</a>
													</xsl:otherwise>
												</xsl:choose>
