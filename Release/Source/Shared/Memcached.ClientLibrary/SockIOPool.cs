    ...
    public class SockIOPool
    {
        ...
        public void Initialize() 
		{
			...
			if(_initialized
				...
				&& _busyPool != null) 
			{
				Logging.Error(GetLocalizedString("initializing initialized pool"));
				return;
			}
			...
			Logging.Verbose(GetLocalizedString("initializing pool")
			...
			{
				Logging.Error(GetLocalizedString("initialize with no servers"));
			...
			for(int i = 0; i < _servers.Count; i++) 
			{
				...
				if(_weights != null && _weights.Count > i) 
				{
					for(int k = 0; k < ((int)_weights[i]); k++) 
					{
						...
						Logging.Verbose("Added " + _servers[i] + " to server bucket");
					}
				}
				else 
				{
				...
				Logging.Verbose(GetLocalizedString("create initial connections").Replace("$$InitConns$$", InitConnections.ToString(new NumberFormatInfo())).Replace("$$Servers[i]$$", Servers[i].ToString()));
				...
				for(int j = 0; j < _initConns; j++) 
				{
					...
					if(socket == null) 
					{
						Logging.Error(GetLocalizedString("failed to connect").Replace("$$Servers[i]$$", Servers[i].ToString()).Replace("$$j$$", j.ToString(new NumberFormatInfo())));
						break;
					}

					AddSocketToPool(_availPool, (string)_servers[i], socket);
					Logging.Verbose(GetLocalizedString("created and added socket").Replace("$$ToString$$", socket.ToString()).Replace("$$Servers[i]$$", Servers[i].ToString()));
        ...
        protected SockIO CreateSocket(string host)
        {
			...
			try
			{
				...
				{
					Logging.Error(GetLocalizedString("failed to get socket").Replace("$$Host$$", host));
					try
					{
						socket.TrueClose();
					}
					...
					{
						Logging.Error(GetLocalizedString("failed to close socket on host").Replace("$$Host$$", host), ex);
						socket = null;
					}
				}
			}
			...
			{
				Logging.Error(GetLocalizedString("failed to get socket").Replace("$$Host$$", host), ex);
				socket = null;
			}
            ...
            if(socket == null)
            {
				...
				Logging.Verbose(GetLocalizedString("ignoring dead host").Replace("$$Host$$", host).Replace("$$Expire$$", expire.ToString(new NumberFormatInfo())));
            ...
            {
				Logging.Verbose(GetLocalizedString("created socket").Replace("$$ToString$$", socket.ToString()).Replace("$$Host$$", host));
        ...
        public SockIO GetSock(string key, object hashCode)
        {
			...
			Logging.Verbose(GetLocalizedString("cache socket pick").Replace("$$Key$$", key).Replace("$$HashCode$$", hashCodeString));


            if (key == null || key.Length == 0)
            {
				Logging.Verbose(GetLocalizedString("null key"));
                return null;
            }

            if(!_initialized)
            {
				Logging.Error(GetLocalizedString("get socket from uninitialized pool"));
                return null;
            }
            ...
            while(tries++ <= _buckets.Count)
            {
				...
				Logging.Verbose(GetLocalizedString("cache choose").Replace("$$Bucket$$", _buckets[bucket].ToString()).Replace("$$Key$$", key));
        ...
        public SockIO GetConnection(string host)
        {
            ...
            if(_availPool != null && !(_availPool.Count == 0))
            {
                ...
                if(aSockets != null && !(aSockets.Count == 0))
                {
                    foreach(SockIO socket in new IteratorIsolateCollection(aSockets.Keys))
                    {
                        if(socket.IsConnected)
                        {
							Logging.Verbose(GetLocalizedString("move socket").Replace("$$Host$$", host).Replace("$$Socket$$", socket.ToString())); 
                        ...
                        else
                        {
							...
							Logging.Error(GetLocalizedString("socket not connected").Replace("$$Host$$", host).Replace("$$Socket$$", socket.ToString()));
			...
			Logging.Verbose(GetLocalizedString("creating sockets").Replace("$$Create$$", create.ToString(new NumberFormatInfo())));
            ...
            if(pool.ContainsKey(host))
            {
                ...
                if(sockets != null && sockets.Count > 0)
                {
                    foreach(SockIO socket in new IteratorIsolateCollection(sockets.Keys))
                    {
                        ...
                        {
							Logging.Error(GetLocalizedString("failed to close socket"), ioe);
                        }
        ...
        public void CheckIn(SockIO socket, bool addToAvail)
        {
			...
			Logging.Verbose("Calling check-in on socket: " + socket.ToString() + " for host: " + host);
			

            // remove from the busy pool
			Logging.Verbose("Removing socket (" + socket.ToString() + ") from busy pool for host: " + host);
			RemoveSocketFromPool(_busyPool, host, socket);

            // add to avail pool
            if(addToAvail && socket.IsConnected)
            {
				Logging.Verbose("Returning socket (" + socket.ToString() + " to avail pool for host: " + host);
				AddSocketToPool(_availPool, host, socket);
            }
        }
        ...
        protected static void ClosePool(Hashtable pool)
        {
            ...
            foreach(string host in pool.Keys)
            {
                ...
                foreach(SockIO socket in new IteratorIsolateCollection(sockets.Keys))
                {
                    ...
                    {
						Logging.Error(GetLocalizedString("failed to true close").Replace("$$ToString$$", socket.ToString()).Replace("$$Host$$", host), ioe);
                    }
        ...
        {
			Logging.Verbose(GetLocalizedString("start socket pool shutdown"));
			if(_maintenanceThread != null && _maintenanceThread.IsRunning)
                StopMaintenanceThread();

			Logging.Verbose("Closing all internal pools.");
            ...
            _hostDead = null;
            _initialized = false;
			Logging.Verbose(GetLocalizedString("end socket pool shutdown"));
        ...
        protected void StartMaintenanceThread()
        {
            if(_maintenanceThread != null)
            {
                ...
                {
					Logging.Error(GetLocalizedString("main thread running"));
                }
                else
                {
        ...
        {
			Logging.Verbose(GetLocalizedString("start self maintenance"));
            ...
            foreach(string host in new IteratorIsolateCollection(_availPool.Keys))
            {
				...
				Logging.Verbose(GetLocalizedString("size of available pool").Replace("$$Host$$", host).Replace("$$Sockets$$", sockets.Count.ToString(new NumberFormatInfo())));
                ...
                if(sockets.Count < _minConns)
                {
					...
					Logging.Verbose(GetLocalizedString("need to create new sockets").Replace("$$Need$$", need.ToString(new NumberFormatInfo())).Replace("$$Host$$", host));
                ...
                else if(sockets.Count > _maxConns)
                {
					...
					Logging.Verbose(GetLocalizedString("need to remove spare sockets").Replace("$$NeedToClose$$", needToClose.ToString(new NumberFormatInfo()).Replace("$$Host$$", host)));
                    ...
                    foreach(SockIO socket in new IteratorIsolateCollection(sockets.Keys))
                    {
                        ...
                        {
							Logging.Verbose(GetLocalizedString("removing stale entry"));
            ...
            foreach(string host in _busyPool.Keys)
            {
				...
				Logging.Verbose(GetLocalizedString("size of busy pool").Replace("$$Host$$", host).Replace("$$Sockets$$", sockets.Count.ToString(new NumberFormatInfo())));
                ...
                foreach(SockIO socket in new IteratorIsolateCollection(sockets.Keys))
                {
                    ...
                    {
						Logging.Error(GetLocalizedString("removing hung connection").Replace("$$Time$$", (new TimeSpan(DateTime.Now.Ticks - hungTime.Ticks).TotalMilliseconds).ToString(new NumberFormatInfo())));
                        try
                        {
                            socket.TrueClose();
                        }
			...
			Logging.Verbose(GetLocalizedString("end self maintenance"));
        }
        ...
        private class MaintenanceThread
        {
            ...
            private void Maintain()
            {
                while(!_stopThread)
                {
                    ...
                    {
						Logging.Error(GetLocalizedString("maintenance thread choked"), ex);
                    }
                }
            }
