...
namespace Memcached.ClientLibrary
{
	...
	public class MemcachedClient 
	{
		...
		public bool Delete(string key, object hashCode, DateTime expiry) 
		{
			if(key == null) 
			{
    			Logging.Error(GetLocalizedString("null key delete"));
				return false;
			}
			...
			try 
			{
				...
				if(DELETED == line) 
				{
						Logging.Verbose(GetLocalizedString("delete success").Replace("$$Key$$", key));
				...
				{
						Logging.Verbose(GetLocalizedString("delete key not found").Replace("$$Key$$", key));
				}
				else 
				{
						Logging.Error(GetLocalizedString("delete key error").Replace("$$Key$$", key).Replace("$$Line$$", line));

				}
			}
			catch(IOException e) 
			{
					Logging.Error(GetLocalizedString("delete IOException"), e);
				...
				{
					Logging.Error(GetLocalizedString("failed to close some socket").Replace("$$Socket$$", sock.ToString()), ioe);
				}
		...
		private bool Set(string cmdname, string key, object obj, DateTime expiry, object hashCode, bool asString) 
		{
			...
			if(NativeHandler.IsHandled(obj)) 
			{
				if(asString) 
				{
                    if(obj != null)
                    {
						...
						Logging.Verbose(GetLocalizedString("set store data as string").Replace("$$Key$$", key).Replace("$$Class$$", obj.GetType().Name));
                        ...
                        catch(ArgumentException ex)
                        {
							...
							Logging.Error(GetLocalizedString("set invalid encoding").Replace("$$Encoding$$", _defaultEncoding), ex);
				...
				else 
				{
					...
					Logging.Verbose(GetLocalizedString("set store with native handler"));
					...
					catch(ArgumentException e) 
					{
							...
							Logging.Error(GetLocalizedString("set failed to native handle object"), e);
			...
			else 
			{
                if(obj != null)
                {
					...
					Logging.Verbose(GetLocalizedString("set serializing".Replace("$$Key$$", key).Replace("$$Class$$", obj.GetType().Name)));
                    ...
                    catch(IOException e)
                    {
                        ...
                        // we bail
							...
							Logging.Error(GetLocalizedString("set failed to serialize").Replace("$$Object$$", obj.ToString()), e);
			...
			if(_compressEnable && length > _compressThreshold) 
			{
					...
					Logging.Verbose(GetLocalizedString("set trying to compress data"));
					Logging.Verbose(GetLocalizedString("set size prior").Replace("$$Size$$", length.ToString(new NumberFormatInfo())));
				...
				catch(IOException e) 
				{
						...
						Logging.Error(GetLocalizedString("set compression failure"), e);
			...
			{
				// exception thrown
					Logging.Error(GetLocalizedString("set IOException"), e);
		...
		public long GetCounter(string key, object hashCode) 
		{
			if(key == null) 
			{
				...
				Logging.Error(GetLocalizedString("getcounter null key"));
				return -1;
			}
			...
			catch(ArgumentException) 
			{
                ...
                Logging.Error(GetLocalizedString("getcounter counter not found").Replace("$$Key$$", key));
			}
		...
		private long IncrementOrDecrement(string cmdname, string key, long inc, object hashCode) 
		{
			...
			try 
			{
				string cmd = cmdname + " " + key + " " + inc + "\r\n";
					...
					Logging.Verbose(GetLocalizedString("incr-decr command").Replace("$$Cmd$$", cmd));
				...
				else if(NOTFOUND == line) 
				{
						...
						Logging.Verbose(GetLocalizedString("incr-decr key not found").Replace("$$Key$$", key));
				} 
				else 
				{
						Logging.Error(GetLocalizedString("incr-decr key error").Replace("$$Key$$", key));
				}
			}
			catch(IOException e) 
			{
				// exception thrown
					Logging.Error(GetLocalizedString("incr-decr IOException"), e);
				...
				catch(IOException) 
				{
						...
						Logging.Error(GetLocalizedString("failed to close some socket").Replace("$$Socket$$", sock.ToString()));
				}
		...
		public object Get(string key, object hashCode, bool asString) 
		{
			...
			{
				string cmd = "get " + key + "\r\n";
					Logging.Verbose(GetLocalizedString("get memcached command").Replace("$$Cmd$$", cmd));
				...
				LoadItems(sock, hm, asString);
					...
					// debug code
					Logging.Verbose(GetLocalizedString("get memcached result").Replace("$$Results$$", hm.Count.ToString(new NumberFormatInfo())));
			...
			{
				// exception thrown
					Logging.Error(GetLocalizedString("get IOException").Replace("$$Key$$", key), e);
        ...
        public Hashtable GetMultiple(string[] keys, int[] hashCodes, bool asString) 
		{
			...
			for(int i = 0; i < keys.Length; ++i) 
			{
				...
				Logging.Verbose(GetLocalizedString("getmultiple socket count").Replace("$$Sockets$$", sockKeys.Count.ToString(new NumberFormatInfo())));
			...
			foreach(string host in sockKeys.Keys) 
			{
				...
				try 
				{
					string cmd = "get" + (StringBuilder) sockKeys[ host ] + "\r\n";
						Logging.Verbose(GetLocalizedString("getmultiple memcached command").Replace("$$Cmd$$", cmd));
					sock.Write(UTF8Encoding.UTF8.GetBytes(cmd));
					sock.Flush();
				...
				{
					// exception thrown
						Logging.Error(GetLocalizedString("getmultiple IOException"), e);
			...
			foreach(string host in toRemove)
			{
				...
				Logging.Verbose(GetLocalizedString("getmultiple results").Replace("$$Results$$", ret.Count.ToString(new NumberFormatInfo())));
			return ret;
		}
		...
		private void LoadItems(SockIO sock, Hashtable hm, bool asString) 
		{
			...
			{
				string line = sock.ReadLine();
				Logging.Verbose(GetLocalizedString("loaditems line").Replace("$$Line$$", line));
				...
				if(line.StartsWith(VALUE)) 
				{
                    ...
                    Logging.Verbose(GetLocalizedString("loaditems header").Replace("$$Key$$", key).Replace("$$Flags$$", flag.ToString(new NumberFormatInfo())).Replace("$$Length$$", length.ToString(new NumberFormatInfo())));
					...
					if((flag & F_COMPRESSED) != 0) 
					{
						...
						{
							Logging.Error(GetLocalizedString("loaditems uncompression IOException").Replace("$$Key$$", key), e);
					...
					if((flag & F_SERIALIZED) == 0) 
					{
						if(_primitiveAsString || asString) 
						{
							...
							Logging.Verbose(GetLocalizedString("loaditems retrieve as string"));
						...
						else 
						{
							...
							{
								Logging.Error(GetLocalizedString("loaditems deserialize error").Replace("$$Key$$", key), e);
					...
					else 
					{
						...
						try 
						{
							...
							Logging.Verbose(GetLocalizedString("loaditems deserializing").Replace("$$Class$$", o.GetType().Name));
							
						}
						catch(SerializationException e) 
						{
							Logging.Error(GetLocalizedString("loaditems SerializationException").Replace("$$Key$$", key), e);
				...
				{
					Logging.Verbose(GetLocalizedString("loaditems finished"));
					break;
				}
			}
		}
		...
		public bool FlushAll(ArrayList servers) 
		{
			...
			if(pool == null) 
			{
				Logging.Error(GetLocalizedString("unable to get socket pool"));
				return false;
			}
			...
			{
				Logging.Error(GetLocalizedString("flushall no servers"));
				return false;
			}
			...
			for(int i = 0; i < servers.Count; i++) 
			{
				...
				if(sock == null) 
				{
					Logging.Error(GetLocalizedString("unable to connect").Replace("$$Server$$", servers[i].ToString()));
				...
				{
					Logging.Error(GetLocalizedString("flushall IOException"), e);
		...
		public Hashtable Stats(ArrayList servers) 
		{
			...
			if(servers == null || servers.Count <= 0) 
			{
				Logging.Error(GetLocalizedString("stats no servers"));
				return null;
			}
			...
			for(int i = 0; i < servers.Count; i++) 
			{
				...
				try 
				{
					...
					{
						string line = sock.ReadLine();
						Logging.Verbose(GetLocalizedString("stats line").Replace("$$Line$$", line));
						...
						if(line.StartsWith(STATS)) 
						{
							...
							Logging.Verbose(GetLocalizedString("stats success").Replace("$$Key$$", key).Replace("$$Value$$", val));
						...
						else if(END == line) 
						{
							...
							Logging.Verbose(GetLocalizedString("stats finished"));
							break;
						}
				...
				{
					Logging.Error(GetLocalizedString("stats IOException"), e);
