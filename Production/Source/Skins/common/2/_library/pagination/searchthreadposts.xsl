...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
		...
		<xsl:template match="SEARCHTHREADPOSTS" mode="library_pagination_searchthreadposts">
			<xsl:if test="@TOTALPOSTCOUNT > @COUNT">
					<ul class="pagination">
						<li class="first dna-button">
								<xsl:choose>
											<xsl:when test="@SKIPTO > 0">
													<a href="{$root}/searchposts?searchtext={@SEARCHTERM}&amp;skip=0">
							...
							<li class="previous dna-button">
									<xsl:choose>
											<xsl:when test="@SKIPTO > 0">
													<a href="{$root}/searchposts?searchtext={@SEARCHTERM}&amp;skip={@SKIPTO - @COUNT}">
															<span class="arrow">
							...
							<li class="next dna-button">
							 ...
							 <li class="last dna-button">
								<xsl:choose>
											<xsl:when test="@SKIPTO + @COUNT &lt; @TOTALPOSTCOUNT">
												<xsl:choose>
													<xsl:when test="@TOTALPOSTCOUNT mod @COUNT =0">
														<a href="{$root}/searchposts?searchtext={@SEARCHTERM}&amp;skip={(floor(@TOTALPOSTCOUNT div @COUNT)-1) * @COUNT}">
															<xsl:text> Last</xsl:text>
														</a>
													</xsl:when>
													<xsl:otherwise>
														<a href="{$root}/searchposts?searchtext={@SEARCHTERM}&amp;skip={(floor(@TOTALPOSTCOUNT div @COUNT) * @COUNT)}">
															<xsl:text> Last</xsl:text>
														</a>
													</xsl:otherwise>
												</xsl:choose>
		...
		<xsl:template match="SEARCHTHREADPOSTS" mode="library_pagination_pagelist">
			<xsl:param name="totalPages">
				<!--ceil by floor() + 1-->
				<xsl:choose>
					<xsl:when test="@TOTALPOSTCOUNT mod @COUNT =0">
						<xsl:value-of select="floor(@TOTALPOSTCOUNT div @COUNT)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="floor(@TOTALPOSTCOUNT div @COUNT) + 1"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:param>
				...
				<xsl:if test="($totalPages > 1) and ($counter > ($currentPage - 5) ) and ($counter &lt; ($currentPage + 5) )">
						<li>
								...
								<a href="{$root}/searchposts?searchtext={@SEARCHTERM}&amp;skip={@COUNT * ($counter - 1)}">
