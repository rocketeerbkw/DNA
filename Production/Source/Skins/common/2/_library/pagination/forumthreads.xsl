...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
    ...
    <xsl:template match="FORUMTHREADS" mode="library_pagination_forumthreads">
	    <xsl:if test="@TOTALTHREADS > @COUNT">
	        <ul class="pagination">
	            <li class="first dna-button">
	              <xsl:choose>
	                    <xsl:when test="@SKIPTO > 0">
	                        <a href="{$root}/NF{@FORUMID}?skip=0">
	            ...
	            <li class="previous dna-button">
	                <xsl:choose>
	                    <xsl:when test="@SKIPTO > 0">
	                        <a href="{$root}/NF{@FORUMID}?skip={@SKIPTO - @COUNT}">
	                            <span class="arrow">
	            ...
	            <li class="last dna-button">
	              <xsl:choose>
	                    <xsl:when test="@MORE != 0">
	                    	<a href="{$root}/NF{@FORUMID}?skip={(floor(@TOTALTHREADS div @COUNT) * @COUNT)}">
    ...
    <xsl:template match="FORUMTHREADS" mode="library_pagination_pagelist">
	    ...
	    <xsl:if test="($totalPages > 1) and ($counter > 0) and ($counter > ($currentPage - 5) ) and ($counter &lt; ($currentPage + 5)) and ($counter &lt;= $totalPages)">
	          <li>
	              <xsl:if test="$currentPage = $counter">
	                  <xsl:attribute name="class">current</xsl:attribute>
	              </xsl:if>
	              <a href="{$root}/NF{@FORUMID}?skip={@COUNT * ($counter - 1)}">
