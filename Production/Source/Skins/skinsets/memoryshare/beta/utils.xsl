...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="DATEFIELDS_SPECIFIC">
		...
		<p>
			...
			<input type="text" name="{$startname}" id="specDate" value="{$startDateString}" maxlength="10" class="txt"/>
		</p>
	</xsl:template>
	...
	<xsl:template name="DATEFIELDS_FUZZYDATE">
		...
		<p>
			...
			<input type="text" name="startdate" id="approxDateFrom" value="{$startDateString}" maxlength="10" class="txt"/>
		</p>
		<p>
			<label for="approxDateTo">Date to (e.g. 25/10/1976)</label><br/>
			<input type="text" name="enddate" id="approxDateTo" value="{$endDateString}" maxlength="10" class="txt"/>
		</p>
		<p>
	...
	<xsl:template name="sso_typedarticle_signin2">
		<!-- its a template as their needs to be a param variable for the type, it cannot hold the full, can change text easily as well -->
		<xsl:param name="type"/>
		<xsl:param name="s_datemode"/>
		<xsl:param name="startdate"/>
		...
		<xsl:choose>
			<xsl:when test="/H2G2/VIEWING-USER/USER">
				<xsl:value-of select="$root"/>
				<xsl:value-of select="$create_article_link" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="/H2G2/SITE/IDENTITYSIGNIN = 0">
						<xsl:value-of select="concat($sso_rootlogin, $create_article_link)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat($signinlink,  $create_article_link)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template name="SOCIAL_LINKS">
		...
		<p id="bookmark-with">
			Bookmark with: <a href="http://news.bbc.co.uk/1/hi/help/6915817.stm">What are these?</a>
		</p>
		<ul id="bookmarks">
			<li id="icon-del">
				<span></span>
		...
		<p id="email-to-friend">
			<a href="http://www.bbc.co.uk/cgi-bin/navigation/mailto.pl?GO=1&amp;REF={$esc_url}" onclick="popmailwin('http://www.bbc.co.uk/cgi-bin/navigation/mailto.pl?GO=1&amp;REF={$esc_url}','Mailer')">
				<span></span>Email to a friend
			</a>
		</p>
	</xsl:template>
	...
	<xsl:template name="GENERATE_COMBINED_LOCATION_RADIOS_NON_FLASH_CONTENT">
		...
		<div id="nonUKLocation">
			<fieldset>
				...
				-->
				<p><input type="radio" class="radio" name="{$fieldName}" id="otherLocation" value="_location-Non-Uk" onclick="fnUpdateFlashMap()"><xsl:if test="$selectedValue='_location-Non-Uk'"><xsl:attribute name="checked">checked</xsl:attribute></xsl:if></input><label for="otherLocation"> Non-UK location</label></p>
			</fieldset>
		</div>
	</xsl:template>
