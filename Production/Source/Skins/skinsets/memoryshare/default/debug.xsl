...
<xsl:template name="VARIABLEDUMP">
	...
	<xsl:if test="$test_IsAdminUser">
		<div class="variable">you are an admin user</div>
	</xsl:if>
...
<div id="debuggingInfo">
	...
	<xsl:apply-templates select="/H2G2/ARTICLES" mode="debug"/>
	...
	<xsl:apply-templates select="/H2G2/ARTICLESEARCH" mode="debug"/>
	...
	<xsl:apply-templates select="/H2G2/FORUMTHREADPOSTS" mode="debug"/>
	<xsl:apply-templates select="//HIERARCHYDETAILS" mode="debug"/>
	<xsl:apply-templates select="/H2G2/HOT-PHRASES" mode="debug"/>
	<xsl:apply-templates select="/H2G2/INDEX" mode="debug"/>
	<xsl:apply-templates select="/H2G2/INFO" mode="debug"/>
	...
	<xsl:apply-templates select="/H2G2/SEARCH" mode="debug"/>
	...
	<!--
	$sport: <xsl:value-of select="$sport" /><br />
	$sport_converted: <xsl:value-of select="$sport_converted" /><br />
	$competition: <xsl:value-of select="$competition" /><br />
	$banner: <xsl:value-of select="$banner" /><br />
	<br />
	$hometeam: <xsl:value-of select="$hometeam" /><br />
	$awayteam: <xsl:value-of select="$awayteam" /><br />
	$currentteam: <xsl:value-of select="$currentteam" /><br />
	$team: <xsl:value-of select="$team" /><br />
	$team2: <xsl:value-of select="$team2" /><br />
	-->
	...
	$test_IsAdminUser = <xsl:value-of select="$test_IsAdminUser" /><br />
	$test_OwnerIsViewer = <xsl:value-of select="$test_OwnerIsViewer" /><br />	
	...
	<textarea style="width:100%" rows="6"><xsl:copy-of select="/H2G2" /></textarea>
...
<xsl:template match="/H2G2/ARTICLE" mode="debug">
	<div class="debug">
		...
		<br />
		ARTICLEINFO/RELATEDMEMBERS<br />
		<xsl:apply-templates select="//RELATEDMEMBERS" mode="debug"/>
		<br />
		ARTICLEINFO/CRUMBTRAILS
		<xsl:apply-templates select="//CRUMBTRAILS" mode="debug"/>
	</div>
</xsl:template>
...
<!-- RELATEDMEMBERS
used on: articlepage.xsl -->
<xsl:template match="RELATEDMEMBERS" mode="debug">
	<div class="debug">
		<h2>RELATEDMEMBERS</h2>
...
<!-- CRUMBTRAILS
used on: articlepage.xsl -->
<xsl:template match="CRUMBTRAILS" mode="debug">
	<div class="debug">
		<h2>CRUMBTRAILS</h2>
		<xsl:call-template name="tablex3"/>
	</div>		
...
<xsl:template match="/H2G2/ARTICLEFORUM" mode="debug">
	<div class="debug">
		...
		<xsl:apply-templates select="FORUMTHREADS" mode="debug"/>
		<xsl:apply-templates select="FORUMTHREADPOSTS" mode="debug"/>
	</div>		
</xsl:template>
...
<xsl:template match="//FORUMTHREADPOSTS" mode="debug">
	<div class="debug">
		<h2>FORUMTHREADPOSTS</h2>
				@GUESTBOOK:<xsl:value-of select="@GUESTBOOK" /><br />
				@FORUMID:<xsl:value-of select="@FORUMID" /><br />
				@ALERTINSTANTLY:<xsl:value-of select="@ALERTINSTANTLY" /><br />
				@SKIPTO:<xsl:value-of select="@SKIPTO" /><br />
				@COUNT:<xsl:value-of select="@COUNT" /><br />
				@CANREAD:<xsl:value-of select="@CANREAD" /><br />
				@CANWRITE:<xsl:value-of select="@CANWRITE" /><br />
				@FORUMPOSTCOUNT:<xsl:value-of select="@FORUMPOSTCOUNT" /><br />
				@TOTALPOSTCOUNT:<xsl:value-of select="@TOTALPOSTCOUNT" /><br />
				@SITEID:<xsl:value-of select="@SITEID" /><br />
				@DEFAULTCANREAD:<xsl:value-of select="@DEFAULTCANREAD" /><br />
				@DEFAULTCANWRITE:<xsl:value-of select="@DEFAULTCANWRITE" /><br />
					...
					<td>
						<xsl:choose>
							<xsl:when test="/H2G2/@TYPE='ARTICLE'">
								<xsl:value-of select="(../@TOTALPOSTCOUNT - @INDEX)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="(1 + @INDEX)"/>
							</xsl:otherwise>
						</xsl:choose>
					</td>
...
<xsl:template match="/H2G2/ARTICLEHOT-PHRASES" mode="debug">
	<xsl:if test="$DEBUG = 1 or /H2G2/PARAMS/PARAM[NAME = 's_debug']/VALUE = '1'">
		<div class="debug">
				...
				<tr>
				<td><xsl:value-of select="position()"/></td>
				<td><a href="{$articlesearchservice}?contenttype={/H2G2/ARTICLEHOT-PHRASES/@ASSETCONTENTTYPE}&amp;phrase={NAME}"><xsl:value-of select="NAME"/></a></td>
...
<xsl:template match="/H2G2/ARTICLESEARCHPHRASE" mode="debug">
	<xsl:if test="$DEBUG = 1 or /H2G2/PARAMS/PARAM[NAME = 's_debug']/VALUE = '1'">
		<div class="debug">
			...
			<td><a href="A{@H2G2ID}">
			<xsl:choose>
				<xsl:when test="SUBJECT/text()"></xsl:when>
				<xsl:otherwise>NO SUBJECT</xsl:otherwise>
			</xsl:choose>
			<xsl:value-of select="SUBJECT"/></a></td>
			<td><xsl:value-of select="EXTRAINFO/TYPE/@ID"/></td>
			<td><xsl:value-of select="EXTRAINFO/AUTODESCRIPTION"/></td>
			<td><a href="U{EDITOR/USER/USERID}"><xsl:value-of select="EDITOR/USER/USERNAME"/></a></td>
			...
			<td><xsl:value-of select="PHRASES/@COUNT"/>
				<xsl:for-each select="PHRASES/PHRASE"><xsl:text> </xsl:text><a href="{$articlesearchservice}?contenttype={../../MEDIAASSET/@CONTENTTYPE}&amp;phrase={NAME}"><xsl:value-of select="NAME"/></a> <xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>
			</td>
			</tr>
			</xsl:for-each>
			</table>
...
<xsl:template match="/H2G2/DYNAMIC-LISTS" mode="debug">
	<div class="debug"><a name="DYNAMIC-LISTS"/>
			...
			<div style="border:1px solid #999;padding:10px;margin:10px 0;"><strong>LIST  <xsl:value-of select="position()"/></strong><br />
				...
				<xsl:choose>
					<xsl:when test="@LISTTYPE = 'ARTICLES'">
						...
						<td><a href="{$root}A{ARTICLE-ITEM/@H2G2ID}"><xsl:value-of select="ARTICLE-ITEM/SUBJECT"/></a></td>
						<td><xsl:value-of select="ARTICLE-ITEM/@ARTICLESTATUS"/></td>
						<td><a href="{$root}U{ARTICLE-ITEM/AUTHOR/USER/USERID}"><xsl:value-of select="ARTICLE-ITEM/AUTHOR/USER/USERNAME"/></a></td>
...
used on: articlepage.xsl
	: threadspage.xsl -->
<xsl:template match="//FORUMTHREADS" mode="debug">
	<div class="debug">
	<h2>FORUMTHREADS</h2>
		@FORUMID:<xsl:value-of select="@FORUMID" /><br />
		@SKIPTO:<xsl:value-of select="@SKIPTO" /><br />
		@COUNT:<xsl:value-of select="@COUNT" /><br />
		@TOTALTHREADS:<xsl:value-of select="@TOTALTHREADS" /><br />
		@FORUMPOSTCOUNT:<xsl:value-of select="@FORUMPOSTCOUNT" /><br />
...
<xsl:template match="HIERARCHYDETAILS" mode="debug">
	<div class="debug">
		<h2>HIERARCHYDETAILS</h2>
		<xsl:call-template name="tablex3"/><br />
		<br />
		<xsl:call-template name="categorypage_debug"/>
	</div>
</xsl:template>
...
<!-- INDEX
used on: indexpage.xsl -->
<xsl:template match="/H2G2/INDEX" mode="debug">
	<div class="debug">
		/H2G2/INDEX<br />
...
<!-- INFO
infopage.xsl 
info?cmd=art&show=100
info?cmd=conv
-->
<xsl:template match="/H2G2/INFO" mode="debug">
		<div class="debug">
				/H2G2/INFO<br />
				@SHOW: <xsl:value-of select="@SHOW"/><br />
				@COUNT: <xsl:value-of select="@COUNT"/><br />
				...
				<xsl:if test="FRESHESTARTICLES/RECENTARTICLE">
				<!-- info?cmd=art -->
				FRESHESTARTICLES/RECENTARTICLE
				<table cellpadding="0" cellspacing="0" border="1" style="font-size:100%;">
				<tr>
				<th>position</th>
				<th>SUBJECT</th>
				...
				<xsl:for-each select="FRESHESTARTICLES/RECENTARTICLE">
				<tr>
				<td><xsl:value-of select="position()"/></td>
				<td><a href="A{H2G2ID}"><xsl:value-of select="SUBJECT"/></a></td>
				<td><xsl:value-of select="STATUS"/></td>
				<td><xsl:value-of select="DATEUPDATED/DATE/@DAY" />&nbsp;<xsl:value-of select="DATEUPDATED/DATE/@MONTHNAME" />&nbsp;<xsl:value-of select="DATEUPDATED/DATE/@YEAR" /><xsl:text> </xsl:text><xsl:value-of select="DATEUPDATED/DATE/@HOURS" />:<xsl:value-of select="DATEUPDATED/DATE/@MINUTES" />:<xsl:value-of select="DATEUPDATED/DATE/@SECONDS" /> - <xsl:value-of select="DATEUPDATED/DATE/@RELATIVE" /></td>
				...
				<xsl:if test="RECENTCONVERSATIONS/RECENTCONVERSATION">
				<!-- info?cmd=conv -->
				...
				<xsl:for-each select="RECENTCONVERSATIONS/RECENTCONVERSATION">
				<tr>
				<td><xsl:value-of select="position()"/></td>
				<td><a href="F{FORUMID}?thread={THREADID}"><xsl:value-of select="FIRSTSUBJECT"/></a></td>
				<td><xsl:value-of select="DATEPOSTED/DATE/@DAY" />&nbsp;<xsl:value-of select="DATEPOSTED/DATE/@MONTHNAME" />&nbsp;<xsl:value-of select="DATEPOSTED/DATE/@YEAR" /><xsl:text> </xsl:text><xsl:value-of select="DATEPOSTED/DATE/@HOURS" />:<xsl:value-of select="DATEPOSTED/DATE/@MINUTES" />:<xsl:value-of select="DATEPOSTED/DATE/@SECONDS" /> - <xsl:value-of select="DATEPOSTED/DATE/@RELATIVE" /></td>
				</tr>
				</xsl:for-each>
				</table>
...
<xsl:template match="MEDIAASSETBUILDER/MEDIAASSETINFO[ACTION='showusersassets']" mode="debug">
	<xsl:variable name="assetroot">/dna/memoryshare</xsl:variable>
...
<xsl:template match="/H2G2/MEDIAASSETBUILDER/ARTICLEMEDIAASSETINFO" mode="debug">
	<div class="debug">
		...
		<td><xsl:value-of select="MEDIAASSET/PHRASES/@COUNT"/>
			<xsl:for-each select="MEDIAASSET/PHRASES/PHRASE"><xsl:text> </xsl:text><a href="{$articlesearchservice}?&amp;phrase={NAME}"><xsl:value-of select="NAME"/></a> <xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>
			<br /></td>
	...
	<div class="debug">
		...
		MEDIAASSET<br />
			...
			<td valign="top"><b>Key phrases:</b></td><td><xsl:value-of select="PHRASES/@COUNT"/>
				<ul>
					<xsl:for-each select="PHRASES/PHRASE">
					<li><a href="{$articlesearchservice}?contenttype={../../CONTENTTYPE}&amp;phrase={NAME}"><xsl:value-of select="NAME"/></a></li>
					</xsl:for-each>
				</ul></td>
			</tr>				
			<tr>
...
<xsl:template match="/H2G2/ONLINEUSERS" mode="debug">
	<div class="debug">
		/H2G2/ONLINEUSERS<br />
		@THISSITE = <xsl:value-of select="@THISSITE"/><br />
		@THISSITE = <xsl:value-of select="@ORDER-BY"/><br />
		<br />
		ONLINEUSER		
		<table cellpadding="1" cellspacing="0" border="1" style="font-size:100%;">
		<tr>
			...
			<th valign="top">DAYSSINCEJOINED</th>
		</tr>
		<xsl:for-each select="ONLINEUSER">
		<tr>
			<td valign="top"><a href="U{USER/USERID}"><xsl:value-of select="USER/USERNAME"/></a></td>
			<td valign="top"><xsl:value-of select="USER/USERID"/><br /></td>
			<td valign="top"><xsl:value-of select="USER/EDITOR"/><br /></td>
			<td valign="top"><xsl:value-of select="DAYSSINCEJOINED"/><br /></td>
			</tr>
		</xsl:for-each>		
		</table>
		
		<br />WEIRD =  <xsl:value-of select="WEIRD"/>
...
<!-- PHRASES
used on: articlepage.xsl -->
<xsl:template match="/H2G2/PHRASES" mode="debug">
	<div class="debug">
		<h2>/H2G2/PHRASES</h2>
...
<!-- POSTS
used on: morepostspage.xsl -->
<xsl:template match="/H2G2/POSTS" mode="debug">
	<div class="debug">
		/H2G2/POSTS<br />
		USERID <xsl:value-of select="@USERID"/><br />
		<xsl:apply-templates select="//POST-LIST" mode="debug"/>
	</div>
</xsl:template>

<!-- POST-LIST
used on: morepostspage.xsl and userpage.xsl -->
<xsl:template match="//POST-LIST" mode="debug">
	<div class="debug">
		POST-LIST<br />
		@COUNT: <xsl:value-of select="@COUNT"/><br />
		@SKIPTO: <xsl:value-of select="@SKIPTO"/><br />
...
<!-- POSTTHREADFORM
used on: articlepage.xsl -->
<xsl:template match="POSTTHREADFORM" mode="debug">
	<div class="debug">
		<h2>POSTTHREADFORM</h2>
		<xsl:call-template name="tablex3"/>
	</div>		
</xsl:template>
...
<xsl:template match="/H2G2/RECENT-APPROVALS" mode="debug">	
	<div class="debug">
		...
		number of articles on this site: <xsl:value-of select="count(/H2G2/RECENT-APPROVALS/ARTICLE-LIST/ARTICLE[SITEID=$site_number])" /><br />
...
<xsl:template match="/H2G2/RECENT-ENTRIES" mode="debug">		
	<div class="debug">
		...
		number of articles on this site: <xsl:value-of select="count(/H2G2/RECENT-ENTRIES/ARTICLE-LIST/ARTICLE[SITEID=$site_number])" /><br />
...
<xsl:template match="/H2G2/RECENT-POSTS" mode="debug">		
	<div class="debug">
		...
		number of posts: <xsl:value-of select="count(/H2G2/RECENT-POSTS/POST-LIST/POST)" /><br />
		number of posts on this site: <xsl:value-of select="count(/H2G2/RECENT-POSTS/POST-LIST/POST[SITEID=$site_number])" /><br />
...
<!-- SEARCH
used on: searchpage.xsl -->
<xsl:template match="/H2G2/SEARCH" mode="debug">
	<div class="debug">
		/H2G2/SEARCH<br />
		 @ACTION = <xsl:value-of select="@ACTION"/>
		<xsl:apply-templates select="SEARCHRESULTS" mode="debug"/>
		<xsl:apply-templates select="FUNCTIONALITY" mode="debug"/>
	</div>
</xsl:template>
	...
	<!-- FUNCTIONALITY
	used on: searchpage.xsl -->
	<xsl:template match="FUNCTIONALITY" mode="debug">
		<div class="debug">
			FUNCTIONALITY<br />
			<xsl:call-template name="tablex3"/>
		</div>
...
<xsl:template name="categorypage_debug">
	<xsl:if test="$DEBUG = 1 or /H2G2/PARAMS/PARAM[NAME = 's_debug']/VALUE = '1'">
		...
		DISPLAYNAME = <a href="C{/H2G2/HIERARCHYDETAILS/@NODEID}"><xsl:value-of select="/H2G2/HIERARCHYDETAILS/DISPLAYNAME" /></a><br />
		@NODEID = <xsl:value-of select="/H2G2/HIERARCHYDETAILS/@NODEID" /><br />
		ANCESTOR[1] = <a href="C{/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[1]/NODEID}"><xsl:value-of select="/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[1]/NAME"/></a><br />
		ANCESTOR[2] = <a href="C{/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[2]/NODEID}"><xsl:value-of select="/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[2]/NAME"/></a><br />
		ANCESTOR[3] = <a href="C{/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[3]/NODEID}"><xsl:value-of select="/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[3]/NAME"/></a><br />
		ANCESTOR[4] = <a href="C{/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[4]/NODEID}"><xsl:value-of select="/H2G2/HIERARCHYDETAILS/ANCESTRY/ANCESTOR[4]/NAME"/></a><br />
		<br />
		<br />
...
-->
	...
	<div id="debugformWrapper">
	
		<form method="post" action="http://www.bbc.co.uk/cgi-bin/cgiemail/606/2/includes/error_form.txt" name="debugform">
		<input type="hidden" name="success" value="http://www0.bbc.co.uk/606/2/thanks.html"/>
		<h2>REPORT A BUG</h2>
		<table cellpadding="5">
		<tr><td>From:</td><td><input type="text" name="from" /></td></tr>
		<tr><td>Subject:</td><td><input type="text" name="subject" /></td></tr>
		<tr><td>Error Class:</td><td>
