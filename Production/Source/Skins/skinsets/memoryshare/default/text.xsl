...
<xsl:template name="m_articlereferredtext">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<p>This memory has been temporarily hidden, because a member of our Moderation Team has referred it to the Community Team for a decision as to whether it contravenes the <A xsl:use-attribute-sets="nm_articlereferredtext2" HREF="{$root}HouseRules">House Rules</A> in some way. We will do everything we can to ensure that a decision is made as quickly as possible.</p>
		</xsl:otherwise>
	</xsl:choose>
	</div>
</xsl:template>
