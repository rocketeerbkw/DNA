...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" version="1.0" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:template name="ARTICLESEARCH_MAINBODY">
		...
		<xsl:choose>
			<!--[FIXME: remove]
			<xsl:when test="$s_mode = 'basic'">
				<xsl:apply-templates select="/H2G2[@TYPE='ARTICLESEARCH']" mode="basic_search"/>
			...
			<xsl:otherwise>
				<xsl:if test="not(/H2G2/PARAMS/PARAM[NAME='s_usersubmit']/VALUE)">
					<!-- FLASH_TIMELINE contains both the flash and the flat list -->
					<xsl:call-template name="FLASH_TIMELINE"/>
					...
					<div id="AS_C_Loading">
						<img src="{$imagesource}loading.gif" alt="Loading.."/>
						<p>
							<a href="javascript:_AS_C_cancelRequest()">Cancel</a>
						</p>
					</div>
					-->
	...
	<xsl:template name="SEARCH_URL">
		...
		<xsl:variable name="url">
			...
			<xsl:if test="/H2G2/ARTICLESEARCH/@SORTBY &gt; 0">
			...
			<xsl:if test="/H2G2/ARTICLESEARCH/PHRASES">
				<xsl:for-each select="/H2G2/ARTICLESEARCH/PHRASES/PHRASE">
					<xsl:text>&amp;phrase=</xsl:text>
	...
	<xsl:template name="FLASH_TIMELINE">
		<!--[FIXME: remove]
		<xsl:variable name="tl_flashvars">
			<xsl:text>defaultStartDate=</xsl:text>
			<xsl:call-template name="IMPLODE_DATE">
				<xsl:with-param name="day" select="/H2G2/ARTICLESEARCH/DATERANGESTART/DATE/@DAY"/>
				<xsl:with-param name="month" select="/H2G2/ARTICLESEARCH/DATERANGESTART/DATE/@MONTH"/>
				<xsl:with-param name="year" select="/H2G2/ARTICLESEARCH/DATERANGESTART/DATE/@YEAR"/>
				<xsl:with-param name="intFormat">yes</xsl:with-param>
			</xsl:call-template>
			<xsl:text>&amp;defaultEndDate=</xsl:text>
			<xsl:call-template name="IMPLODE_DATE">
				<xsl:with-param name="day" select="/H2G2/ARTICLESEARCH/DATERANGEEND/DATE/@DAY"/>
				<xsl:with-param name="month" select="/H2G2/ARTICLESEARCH/DATERANGEEND/DATE/@MONTH"/>
				<xsl:with-param name="year" select="/H2G2/ARTICLESEARCH/DATERANGEEND/DATE/@YEAR"/>
		...
		<xsl:if test="$astotal &gt; 0">
			<xsl:variable name="flashQueryStr">
				<!-- dummy parameter to simplify ?/& logic. stands for konker=skill -->
				<xsl:text>?k=s</xsl:text>
				<xsl:if test="/H2G2/PARAMS/PARAM[NAME='s_dateview']">
					<xsl:text>&amp;initialViewDepth=</xsl:text>
					<xsl:value-of select="/H2G2/PARAMS/PARAM[NAME='s_dateview']/VALUE"/>
					<xsl:text>&amp;defaultStartDate=</xsl:text>
					<xsl:call-template name="IMPLODE_DATE">
						<xsl:with-param name="day" select="/H2G2/ARTICLESEARCH/DATERANGESTART/DATE/@DAY"/>
						<xsl:with-param name="month" select="/H2G2/ARTICLESEARCH/DATERANGESTART/DATE/@MONTH"/>
				...
				<!--
				<xsl:if test="/H2G2/PARAMS/PARAM[NAME='s_phrase']">
					<xsl:text>&amp;initialPhrase=</xsl:text>
					<xsl:value-of select="translate(/H2G2/PARAMS/PARAM[NAME='s_phrase']/VALUE, ' ', '+')"/>
					<xsl:text>%20</xsl:text>
					<xsl:value-of select="translate(/H2G2/PARAMS/PARAM[NAME='s_location']/VALUE, ' ', '+')"/>
				</xsl:if>
				-->
				<xsl:if test="/H2G2/ARTICLESEARCH/PHRASES/@COUNT &gt; 0">
					<xsl:variable name="phrases">
						<xsl:for-each select="/H2G2/ARTICLESEARCH/PHRASES/PHRASE">
							...
							<xsl:if test="position() != last()">
								<xsl:value-of select="$keyword_sep_char_url_enc"/>
							</xsl:if>
						</xsl:for-each>
					</xsl:variable>
					
					<xsl:text>&amp;initialPhrase=</xsl:text>
					<xsl:value-of select="$phrases"/>
					<!--
					<xsl:text>%20</xsl:text>
					<xsl:value-of select="translate(/H2G2/PARAMS/PARAM[NAME='s_location']/VALUE, ' ', '+')"/>
					-->
				...
				<xsl:choose>
					<xsl:when test="/H2G2/ARTICLESEARCH/@DATESEARCHTYPE = 1">
						<xsl:text>&amp;defaultDateSearchType=1</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>&amp;defaultDateSearchType=2</xsl:text>
					</xsl:otherwise>
				</xsl:choose>

				<xsl:if test="/H2G2/ARTICLESEARCH/@SORTBY">
					<xsl:text>&amp;defaultArticleSortBy=</xsl:text>
					<xsl:value-of select="/H2G2/ARTICLESEARCH/@SORTBY"/>
				</xsl:if>
			</xsl:variable>
			...
			<div id="flashTimeline">
				<script language="JavaScript" type="text/javascript">
					var timelineUrl = "<xsl:value-of select="concat($flashsource, 'memoryshareTimeline.swf', $flashQueryStr)"/>";
				</script>
				<script language="JavaScript" type="text/javascript">
				<![CDATA[
					insertFlashTimeline();
		...
		<div id="topPage">
			<div>
				<xsl:attribute name="class">
					<xsl:choose>
						<xsl:when test="$articlesearch_browse_mode = 2">left2</xsl:when>
			...
			<div>
				...
				<form method="GET" action="{$articlesearchservice}" id="browseForm">
					...
					<fieldset id="search">
						...
						<div class="left">
							<label for="phrase">keyword (e.g. fishing)</label><br/>
			...
			<script type="text/javascript">
				if (dna_articlesearch_total &gt; 0 &amp;&amp; !getViewHTMLCookie()) {
					document.write('<div id="viewLabel"><h3><xsl:value-of select="$js_view_name"/></h3></div>');
				}
			</script>
			<div id="helpLink">
	...
	<xsl:template name="CONTEXT_INCLUDE">
		...
		<xsl:choose>
			...
			<xsl:when test="$mode = 'ARTICLE'">
				...
				<xsl:if test="$article_date_range_type = 1">
					<xsl:if test="not(/H2G2/ERROR)">
	...
	<xsl:template match="ARTICLES">
		...
		<script language="javascript" type="text/javascript">
			var dna_articlesearch_total = '<xsl:value-of select="$astotal"/>';
		</script>
			...
			<div class="padWrapper">
				<div class="paginationWrapper">
					<div class="barStrong">
						<p class="left">
							<xsl:choose>
								<xsl:when test="/H2G2/PARAMS/PARAM[NAME='s_from']/VALUE = 'advanced_search'">
									<a href="{$advanced_search_url}">&lt;&lt; back</a>
								</xsl:when>
								<xsl:otherwise>
									<xsl:if test="$astotal = 0">
										<script language="javascript" type="text/javascript">
											//writeLastSearchLink('&lt;&lt; Back to previous search');
										</script>
										<xsl:if test="/H2G2/PARAMS/PARAM[NAME='s_last_search']/VALUE">
											<a>
												<xsl:attribute name="href">
													<xsl:value-of select="/H2G2/PARAMS/PARAM[NAME='s_last_search']/VALUE"/>
												</xsl:attribute>
												<xsl:text>&lt;&lt; Back to previous search</xsl:text>
											</a>
										</xsl:if>
									</xsl:if>
								</xsl:otherwise>
							</xsl:choose>
							<xsl:text><!--[FIXME: would like &nbsp; here?]--></xsl:text>
						</p>
						<p class="right">
							<!--[FIXME: not needed here?]
							<a href="#">View all memories</a>
							-->
						</p>
						<div class="clr"/>
					</div>
					...
					<div class="barPale">
						<p class="left"><xsl:apply-templates select="/H2G2/ARTICLESEARCH" mode="results_total"/></p>
						<p class="right">
							<xsl:if test="$astotal != 0">
				...
				<xsl:choose>
					<xsl:when test="$astotal = 0">
						<div class="noResults">
						<xsl:choose>
							<xsl:when test="../DATERANGESTART and ../DATERANGEEND">
								<xsl:variable name="isSpecificDate">
									<xsl:call-template name="IS_NEXT_DAY">
								...
								<p>
									<xsl:choose>
										<xsl:when test="$isSpecificDate='yes'">
												...
												<xsl:attribute name="href">
													<xsl:call-template name="sso_typedarticle_signin">
														...
														<xsl:with-param name="startdate">
															<xsl:call-template name="IMPLODE_DATE">
																<xsl:with-param name="day" select="../DATERANGESTART/DATE/@DAY"/>
																<xsl:with-param name="month" select="../DATERANGESTART/DATE/@MONTH"/>
																<xsl:with-param name="year" select="../DATERANGESTART/DATE/@YEAR"/>
															</xsl:call-template>
														</xsl:with-param>
													</xsl:call-template>														
												</xsl:attribute>
	...
	<xsl:template match="ARTICLE" mode="article_list_item">
		...
		<li>
					...
					<xsl:when test="EXTRAINFO/ALTNAME != ''">
						...
						<xsl:choose>
							...
							<xsl:when test="EDITOR/USER/USERID">
								<a href="{$root}U{EDITOR/USER/USERID}">U<xsl:value-of select="EDITOR/USER/USERID"/></a>
							</xsl:when>
							<xsl:otherwise>
				...
				<xsl:if test="PHRASES/PHRASE">
					...
					<xsl:for-each select="PHRASES/PHRASE">
						<xsl:if test="not(starts-with(NAME, '_') or NAME='user_memory' or NAME='staff_memory')">
							<xsl:apply-templates select="." mode="search_link"/>
							<xsl:text> </xsl:text>
						</xsl:if>
					</xsl:for-each>
				</xsl:if>
			...
			<p>
				<xsl:if test="DATE-CREATED/DATE/@RELATIVE">
				  <xsl:text>Posted: </xsl:text>
				  <xsl:value-of select="DATE-CREATED/DATE/@RELATIVE"/>
				</xsl:if>
                             
				<xsl:if test="($test_OwnerIsViewer=1)">
					<xsl:text> | </xsl:text>
					<a href="{$root}TypedArticle?aedit=new&amp;type={$cur_list_article_type}&amp;h2g2id={@H2G2ID}{H2G2-ID}">
						<xsl:text>Edit memory</xsl:text>
					</a>
				</xsl:if>

				<!-- delete -->
				<!-- can only delete if user is the memories author or they are an admin user -->
				<xsl:if test="($test_OwnerIsViewer=1) or $test_IsAdminUser">
					<xsl:text> | </xsl:text>
					<a href="{$root}A{@H2G2ID}{H2G2-ID}?s_confirm_del=1">
	...
	<xsl:template match="/H2G2[@TYPE='ARTICLESEARCH']/ARTICLESEARCH" mode="results_total">
		...
		<em><xsl:value-of select="$astotal"/></em>
		<xsl:choose>
			<xsl:when test="$astotal &gt; 1 or $astotal = 0">
				<xsl:text> memories</xsl:text>
			</xsl:when>
	...
	<xsl:variable name="page_block_size">10</xsl:variable>
	...
	<xsl:template match="ARTICLESEARCH" mode="pagination_block_revised">
			...
			<xsl:if test="$block_start_page &lt;= $block_end_page">
				<a href="{$articlesearchservice}?show={@SHOW}&amp;skip={$first_page_skip}{$as_search_params}">
					<xsl:choose>
						<xsl:when test="$block_start_page = $current_page">
							<strong><xsl:value-of select="$block_start_page"/></strong>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template match="/H2G2[@TYPE='ARTICLESEARCH']/ARTICLESEARCH" mode="link_previouspage">
		<a href="{$articlesearchservice}?show={@COUNT}&amp;skip={number(@SKIPTO) - number(@COUNT)}{$as_searchparams}" xsl:use-attribute-sets="mARTICLESEARCH_link_previouspage">
	...
	<xsl:template match="/H2G2[@TYPE='ARTICLESEARCH']/ARTICLESEARCH" mode="link_nextpage">
		<a href="{$articlesearchservice}?show={@COUNT}&amp;skip={number(@SKIPTO) + number(@COUNT)}{$as_searchparams}" xsl:use-attribute-sets="mARTICLESEARCH_link_nextpage">
	...
	<xsl:template match="/H2G2[@TYPE='ARTICLESEARCH']/ARTICLESEARCH" mode="link_firstpage">
		<a href="{$articlesearchservice}?show={@COUNT}&amp;skip=0{$as_searchparams}" xsl:use-attribute-sets="mARTICLESEARCH_link_firstpage">
	...
	<xsl:template match="/H2G2[@TYPE='ARTICLESEARCH']/ARTICLESEARCH" mode="link_lastpage">
		<a href="{$articlesearchservice}?show={@COUNT}&amp;skip={number(@TOTAL) - number(@COUNT)}{$as_searchparams}" xsl:use-attribute-sets="mARTICLESEARCH_link_lastpage">
  ...
  <xsl:attribute-set name="mARTICLESEARCH_link_lastpage"/>
	...
	<xsl:variable name="as_artspsplit" select="10"/>
