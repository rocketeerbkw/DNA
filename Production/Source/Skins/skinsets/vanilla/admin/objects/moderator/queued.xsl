...
<xsl:stylesheet
	...
	<xsl:template match="MODERATIONQUEUES" mode="objects_moderator_queued">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<div>
					<table>
						<tbody>
							<xsl:apply-templates select="MODERATION-QUEUE-SUMMARY[STATE = 'queued']" mode="objects_moderator_queuesummary" />
						</tbody>
					</table>
				</div>				
			</xsl:otherwise>
