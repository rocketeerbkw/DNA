...
WHILE (SELECT COUNT(*) FROM [dbo].[AtheletesDetailsSpecial] WHERE Processed = 0) > 0
BEGIN

	SELECT TOP 1 @Id = ID FROM [dbo].[AtheletesDetailsSpecial] WHERE Processed = 0

	SELECT @TwitterUserID	  = TwitterUserID FROM [dbo].[AtheletesDetailsSpecial] WHERE ID = @Id
	SELECT @TwitterScreenName = TwitterScreenName FROM [dbo].[AtheletesDetailsSpecial] WHERE ID = @Id
	SELECT @TwitterName		  = Name FROM [dbo].[AtheletesDetailsSpecial] WHERE ID = @Id
	...
	UPDATE [dbo].[AtheletesDetailsSpecial] SET Processed = 1 WHERE ID = @Id
