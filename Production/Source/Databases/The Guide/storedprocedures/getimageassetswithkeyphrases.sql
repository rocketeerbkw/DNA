/*********************************************************************************
	...
	create procedure getimageassetswithkeyphrases @keyphraselist VARCHAR(8000),
	...
	Outputs:	Image Asset dataset
	Purpose:	Returns the Image Asset dataset with the given key phrases
	
*********************************************************************************/
CREATE PROCEDURE getimageassetswithkeyphrases @keyphraselist VARCHAR(8000), @siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null
AS
BEGIN	
...
--Get total image assets for site with key phrases.
declare @total int
SELECT @total =  COUNT(*) FROM (SELECT MA.ID FROM udf_splitvarchar(@keyphraselist) as s
	...
	INNER JOIN dbo.ImageAsset img WITH (NOLOCK) ON img.MediaAssetID = MA.ID
...
FROM #tempassets tmp
INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = tmp.AssetID
INNER JOIN
(	
