/*********************************************************************************
	...
	create procedure getusersarticleswithimageassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null as
	...
	Purpose:	Get the details of articles with image assets for a given user
	
*********************************************************************************/
CREATE PROCEDURE getusersarticleswithimageassets @userid int, @firstindex int, @lastindex int, @owner bit = null, @sortbycaption bit = null, @sortbyrating bit = null
AS
BEGIN	
...
IF @owner IS NOT NULL
	BEGIN
		--Get total articles with image assets for the viewing user.
		SELECT @total =  COUNT(*) FROM dbo.ImageAsset I WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = I.MediaAssetID
...
ELSE
	BEGIN
		--Get total articles with passed moderation image assets for user.
		SELECT @total =  COUNT(*) FROM dbo.ImageAsset I WITH(NOLOCK)
				INNER JOIN dbo.MediaAsset MA WITH(NOLOCK) ON MA.ID = I.MediaAssetID
				INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON AMA.MediaAssetID = MA.ID
...
IF @owner IS NOT NULL
	...
	IF @sortbycaption IS NOT NULL
		...
		INSERT INTO #tempusrassets (MediaAssetID)
			...
			FROM dbo.ImageAsset I WITH(NOLOCK) 
			INNER JOIN dbo.MediaAsset AS MA WITH(NOLOCK) ON MA.ID = I.MediaAssetID
			INNER JOIN dbo.ArticleMediaAsset AMA WITH(NOLOCK) ON AMA.MediaAssetID = MA.ID
...
INNER JOIN dbo.ImageAsset I WITH(NOLOCK) ON I.MediaAssetID = tmp.MediaAssetID
