/*********************************************************************************
	...
	create procedure getaudioassetsforsite @siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null

	Author:		Steven Francis - from Martins SP
	Created:	08/11/2005
	Inputs:		@siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null
	Outputs:	Audio Assets and Phrases
	Purpose:	Returns the Audio Assets for a site
...
--Get total audio assets for site.
declare @total int
SELECT @total =  COUNT(*) FROM dbo.AudioAsset AS A WITH(NOLOCK) 
