create PROCEDURE fetchmoderationstatisticsbytype @userid int, @type int =0
AS
...
(	
	...
	FROM (  --No primary key or on group members - this sub-query causes a better plan
			...
			and (@type=0 or dbo.udf_getsiteoptionsetting(s.siteid, 'General', 'SiteType') = convert(varchar(5),@type))
		) as s1
...
UNION ALL
--general-complaints-locked  - still usees status=1 to indicate locked.
(
	SELECT 'generalcomplaint',
		'locked',
		count(*), 
