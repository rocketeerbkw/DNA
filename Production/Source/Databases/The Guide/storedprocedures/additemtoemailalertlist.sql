CREATE PROCEDURE additemtoemailalertlist @itemid int, @itemtype int, @notifytype int, @emailalertlistid uniqueidentifier
AS
...
ELSE
BEGIN
	SELECT @MemberID = MemberID FROM dbo.EmailAlertListMembers WHERE EmailAlertListID = @emailalertlistid AND ItemID = @itemid AND ItemType = @itemtype AND NotifyType = @notifytype
END
