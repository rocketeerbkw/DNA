/*
	...
	Unless a superuser/editor is viewing locked items items will be locked to the viewing user.
...
*/
CREATE PROCEDURE getmoderationpostsmostrecentfirst @userid int, @status int = 0, @alerts int = 0,  @lockeditems int = 0, @fastmod int = 0, @issuperuser bit = 0, @modclassid int = NULL, @postid int = NULL, @duplicatecomplaints bit = 0, @show int = 10
As
...
--Priority is given to items that the viewing user has already locked, then date queued desc (MOST RECENT POSTS FIRST)
...
ORDER BY userlocked DESC,  tm.DateQueued DESC
...
SELECT		tm.ModID,
		...
		te.Parent,
		u.UserId,
		CASE WHEN te.UserName IS NOT NULL AND te.forumid = cf.forumid AND cf.NotSignedinUserID = u.UserID THEN te.UserName + ' (Anonymous User)' ELSE u.UserName END as username,
		u.Status,
		u.FirstNames,
		u.LastName,
		p.PrefStatus,
...
SELECT	tm.ModID,
		...
		u.UserId,
		CASE WHEN pmp.nickname IS NOT NULL AND pmp.forumid = cf.forumid AND cf.NotSignedinUserID = u.UserID THEN pmp.nickname + ' (Anonymous User)' ELSE u.UserName END as username,
		u.Status,
		u.FirstNames,
		u.LastName,
		p.PrefStatus,
