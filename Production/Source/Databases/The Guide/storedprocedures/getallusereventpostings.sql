CREATE PROCEDURE getallusereventpostings @userid int, @showuserhidden bit = 0, @siteid int = null
AS
	...
	WHERE t.UserID = @userid AND f.Type = 'Event'
	AND ( f.VisibleTo IS NULL OR (@showuserhidden = 1 AND te.Hidden = 7) ) --pull out user-hidden events.
