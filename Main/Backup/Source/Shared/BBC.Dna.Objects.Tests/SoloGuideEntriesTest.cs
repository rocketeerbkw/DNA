...
{
    /// <summary>
    ///This is a test class for SoloGuideEntriesTest and is intended
    ///to contain all SoloGuideEntriesTest Unit Tests
    ///</summary>
    [TestClass]
    public class SoloGuideEntriesTest
    {
        ...
        /// <summary>
        ///A test for SoloGuideEntries Constructor
        ///</summary>
        [TestMethod]
        ...
        public void CreateSoloGuideEntriesTest()
        {
            ...
            SetupSoloGuideEntriesMocks(out mocks, out creator, out reader, out reader2, 2);

            SoloGuideEntries soloGuideEntries;

            soloGuideEntries = SoloGuideEntries.CreateSoloGuideEntriesFromDatabase(creator, siteId, 0, 20);
            Assert.AreNotEqual(null, soloGuideEntries);

            XmlDocument xml = Serializer.SerializeToXml(soloGuideEntries);
        }

        /// <summary>
        /// Tests if SoloGuideEntries returns an empty list when there aren't any soloGuideEntries.
        /// </summary>
        [TestMethod]
        public void GetSoloGuideEntries_ReturnsEmptyList()
        {
            // PREPARE THE TEST
            ...
            SetupSoloGuideEntriesMocks(out mocks, out creator, out reader, out reader2, 0);
            ...
            Assert.IsTrue(soloGuideEntries.SoloUsers.Count == 0, "Users found - should be empty");
        }
        ...
        public void CreateSoloGuideEntries_IgnoreCache_CacheIsIgnored()
        {
            ...
            SoloGuideEntries actual = SoloGuideEntries.CreateSoloGuideEntries(cache, creator, siteId, 0, 20, ignoreCache);
        ...
        public void CreateSoloGuideEntries_WithDontIgnoreCache_NotUpToDate_ReturnsValidObject()
        {
            ...
            var soloGuideEntries = mocks.DynamicMock<SoloGuideEntries>();
            soloGuideEntries.Stub(x => x.IsUpToDate(null)).Constraints(Is.Anything()).Return(false);


            var cache = mocks.DynamicMock<ICacheManager>();
            cache.Stub(x => x.GetData("")).Constraints(Is.Anything()).Return(soloGuideEntries);
            cache.Stub(x => x.Add("", null)).Constraints(Is.Anything(), Is.Anything()).Repeat.Once();
...
#region MockSetup
        ...
        /// <summary>
        /// Helper function to set up parameters for CreateSoloGuideEntries call
        /// </summary>
        /// <param name="mocks"></param>
        /// <param name="cache"></param>
        /// <param name="readerCreator"></param>
        /// <param name="viewingUser"></param>
        public void CreateSoloGuideEntries_SetupDefaultMocks(out MockRepository mocks, out ICacheManager cache, out IDnaDataReaderCreator readerCreator, out User viewingUser, out ISite site)
        {
        ...
        private void AddSoloGuideEntriesUserDatabaseRows(IDnaDataReader reader, string suffix)
        {
            ...
            reader.Stub(x => x.GetInt32NullAsZero("Count")).Return(26).Repeat.Once();
            reader.Stub(x => x.GetInt32NullAsZero("Count")).Return(12).Repeat.Once();
