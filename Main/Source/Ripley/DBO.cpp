...
/*********************************************************************************
> DBO::DebugTraceSqlMessages(SQLRETURN rc)
	...
	Returns:	-
	Scope:		-
	Purpose:	Show error messages generated by the latest SQL call on m_hConn

*********************************************************************************/

void DBO::DebugTraceSqlMessages(SQLRETURN rc)
{
#ifdef _DEBUG
	if ((rc == SQL_SUCCESS_WITH_INFO) || (rc == SQL_ERROR))
	{
	    ...
	    i = 1;
	    while ((rc = SQLGetDiagRec(SQL_HANDLE_DBC, m_hConn, i, SqlState, &NativeError,
				Msg, sizeof(Msg), &MsgLen)) != SQL_NO_DATA)
		{
			TDVASSERT(false, (LPCTSTR)Msg);
			i++;
	    }
	}
#endif
