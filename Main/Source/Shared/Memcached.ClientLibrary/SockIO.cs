...
namespace Memcached.ClientLibrary
{
    ...
    public class SockIO
    {
        ...
        {
			Logging.Verbose(GetLocalizedString("true close socket").Replace("$$Socket$$", ToString()).Replace("$$Lifespan$$", DateTime.Now.Subtract(_created).ToString()));
            ...
            if (_socket != null)
            {
                ...
                {
					Logging.Error(GetLocalizedString("error closing socket").Replace("$$ToString$$", ToString()).Replace("$$Host$$", Host), ioe);
                ...
                {
					Logging.Error(GetLocalizedString("error closing socket").Replace("$$ToString$$", ToString()).Replace("$$Host$$", Host), soe);
					errMsg.Append(GetLocalizedString("error closing socket").Replace("$$ToString$$", ToString()).Replace("$$Host$$", Host) + System.Environment.NewLine);
        ...
        {
            // check in to pool
			Logging.Verbose(GetLocalizedString("close socket").Replace("$$ToString$$", ToString()));
			_pool.CheckIn(this);
        }
        ...
        public string ReadLine()
        {
            ...
            {
				Logging.Error(GetLocalizedString("read closed socket"));
        ...
        public void Flush()
        {
            if (_socket == null || !_socket.Connected)
            {
				Logging.Error(GetLocalizedString("write closed socket"));
        ...
        private class ConnectThread
        {
            ...
            private void Connect()
            {
                ...
                {
                    _error = true;
					Logging.Error(GetLocalizedString("socket connection exception"), ex);
					
                }

				Logging.Verbose(GetLocalizedString("connect thread connect").Replace("$$Host$$", _host));
