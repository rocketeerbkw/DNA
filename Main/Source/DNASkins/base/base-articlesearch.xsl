...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:template match="ARTICLESEARCH" mode="c_articlesearch">
	...
	<xsl:template match="ARTICLESEARCH" mode="t_articlesearchbox">
	...
	<xsl:template match="ARTICLES" mode="c_articlesearchresults">
		<xsl:apply-templates select="." mode="r_articlesearchresults"/>
	</xsl:template>
	...
	<a href="{$root}ArticleSearch?&amp;phrase={TERM}&amp;contenttype={../@ASSETCONTENTTYPE}" xsl:use-attribute-sets="mARTICLEHOT-PHRASE_r_articlehotphrases"><xsl:value-of select="TERM"/></a>
	</xsl:template>			
<!--
	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
<xsl:template match="ARTICLESEARCH" mode="c_articleblockdisplay">
		...
		<xsl:if test="(($skip + @SHOW) &lt; @TOTAL) and ($skip &lt; $artsp_upperrange)">
		...
		<xsl:choose>
				...
				<xsl:apply-templates select="." mode="on_articleblockdisplay">
					<xsl:with-param name="url">
						<xsl:choose>
							...
							<xsl:otherwise>
								<a href="{$root}ArticleSearch?skip={$skip}&amp;show={@SHOW}{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_on_articleblockdisplay">
									<xsl:call-template name="t_articleontabcontent">
										<xsl:with-param name="range" select="$PostRange"/>
										<xsl:with-param name="pagenumber" select="substring-after($PostRange, ' - ') div @SHOW"/>
									</xsl:call-template>
			...
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="off_articleblockdisplay">
					<xsl:with-param name="url">
						<a href="{$root}ArticleSearch?skip={$skip}&amp;show={@SHOW}{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_off_articleblockdisplay">
	...
	<xsl:param name="artsp_lowerrange">
				<xsl:value-of select="floor(/H2G2/ARTICLESEARCH/@SKIPTO div ($artspsplit * /H2G2/ARTICLESEARCH/@SHOW)) * ($artspsplit * /H2G2/ARTICLESEARCH/@SHOW)"/>
	</xsl:param>
	...
	<xsl:param name="artsp_upperrange">
		<xsl:value-of select="$artsp_lowerrange + (($artspsplit - 1) * /H2G2/ARTICLESEARCH/@SHOW)"/>
	</xsl:param>
	<!-- 
...
<xsl:template match="ARTICLESEARCH" mode="c_previouspage">
	...
	<xsl:template match="ARTICLESEARCH" mode="c_nextpage">
		<xsl:choose>
			<xsl:when test="(@SKIPTO + @SHOW) &lt; @TOTAL">
	...
	<xsl:template match="ARTICLESEARCH" mode="link_previouspage">
		<a href="{$root}ArticleSearch?show={@SHOW}&amp;skip={number(@SKIPTO) - number(@SHOW)}{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_link_previouspage">
			<xsl:copy-of select="$m_previouspageartsp"/></a><xsl:text> </xsl:text>
	</xsl:template>

	<xsl:template match="ARTICLESEARCH" mode="link_nextpage">
		<a href="{$root}ArticleSearch?show={@SHOW}&amp;skip={number(@SKIPTO) + number(@SHOW)}{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_link_nextpage">
	...
	<xsl:template match="ARTICLESEARCH" mode="link_firstpage">
		<a href="{$root}ArticleSearch?show={@SHOW}&amp;skip=0{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_link_firstpage">
			<xsl:copy-of select="$m_firstpageartsp"/></a><xsl:text> </xsl:text>
	</xsl:template>

	<xsl:template match="ARTICLESEARCH" mode="link_lastpage">
		<a href="{$root}ArticleSearch?show={@SHOW}&amp;skip={number(@TOTAL) - number(@SHOW)}{$artspsearchphrase}" xsl:use-attribute-sets="mARTICLESEARCH_link_lastpage">
