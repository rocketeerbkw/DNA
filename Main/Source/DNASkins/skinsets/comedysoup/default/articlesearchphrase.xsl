...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<!-- DEBUG -->
		...
		<h1>
		<xsl:choose>
			<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/PHRASES/@COUNT > 0">
			<!-- RESULT PAGES -->
				<xsl:choose>
					...
					<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE=0">
					all results
						<xsl:call-template name="RSSICON">
		...
		<div  class="default">
			<div class="col1">
				<div class="margins">
					...
					<div class="assetNavFormBlock">
						<xsl:apply-templates select="/H2G2/ARTICLESEARCHPHRASE" mode="c_articlesearch"/>
					</div>
					<!--// asset navigation form block - select -->
					<br />
					...
					<xsl:choose>
						<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/PHRASES/@COUNT = 0">
							...
							<h2>Current popular key phrases for 
								<xsl:choose>
									<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE=1">
										images
									</xsl:when>
					...
					<!-- dropdown - sort by -->
					<div class="assetNavFormBlock">
						<form name="sortByForm" id="sortByForm" method="GET" action="ArticleSearchPhrase">
							<input type="hidden" name="contenttype" value="{/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE}" />
							<label for="sortBy">Sort by:</label>						
							<select name="articlesortby" id="sortBy">
								<option value="DateUploaded">Most recent</option>
								<option value="Caption">
								<xsl:if test="ARTICLESEARCHPHRASE/ARTICLESEARCH/@SORTBY='Caption'">
									<xsl:attribute name="selected">selected</xsl:attribute>
								</xsl:if>
								A-Z by title</option>
								<option value="Rating">
								<xsl:if test="ARTICLESEARCHPHRASE/ARTICLESEARCH/@SORTBY='Rating'">
									<xsl:attribute name="selected">selected</xsl:attribute>
								</xsl:if>
								Most rated</option>
						    </select>
					...
					<!-- pagination block -->
					<xsl:apply-templates select="/H2G2/ARTICLESEARCHPHRASE" mode="pagination_links"/>
			...
			<div class="col2">
				<div class="margins">
					...
					<xsl:if test="/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE=3">
	...
	<xsl:template match="ARTICLESEARCHPHRASE" mode="r_search">
		<label for="soupSearch">Search <xsl:choose>
							<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE=1">
								images
							</xsl:when>
							<xsl:when test="/H2G2/ARTICLESEARCHPHRASE/ARTICLESEARCH/@CONTENTTYPE=2">
						...
						</xsl:choose>:</label><xsl:text> </xsl:text>
		<!--
  ...
  <xsl:attribute-set name="mARTICLESEARCH_link_firstpage"/>
	...
	-->
	<xsl:template match="ARTICLESEARCHPHRASE" mode="ARTICLESEARCHBOX">
	...
	<xsl:template match="ARTICLESEARCHPHRASE" mode="pagination_links">
		<div class="paginationBlock">
		<xsl:apply-templates select="ARTICLESEARCH" mode="c_previouspage"/>
		<xsl:apply-templates select="ARTICLESEARCH" mode="c_articleblocks"/>
		<xsl:apply-templates select="ARTICLESEARCH" mode="c_nextpage"/>
		</div>
	</xsl:template>
		...
		<li class="smallFB">
			<div class="imageHolder">
				<xsl:if test="MEDIAASSET/@MEDIAASSETID">
					<a href="{$root}A{@H2G2ID}"><xsl:apply-templates select="MEDIAASSET/@MEDIAASSETID" mode="c_articlethumb"/></a>
				</xsl:if>
			</div>
			<div class="content">
				<h2 class="fbHeader"><a href="{$root}A{@H2G2ID}"><xsl:value-of select="SUBJECT"/></a></h2>
	...
	<xsl:template match="ARTICLESEARCH" mode="link_nextpage">
		<span class="next_active">|<b><a href="{$root}ArticleSearchPhrase?show={@COUNT}&amp;skip={number(@SKIPTO) + number(@COUNT)}{$artspsearchphrase}&amp;contenttype={@CONTENTTYPE}&amp;ArticleSortby={@SORTBY}" xsl:use-attribute-sets="mARTICLESEARCH_link_nextpage">
			<xsl:copy-of select="$m_nextpageartsp"/></a></b></span>
	</xsl:template>
	
	<!-- override base files as need to have contenttype= in the querystring -->
	<xsl:template match="ARTICLESEARCH" mode="link_previouspage">
		<span class="prev_active"><b><a href="{$root}ArticleSearchPhrase?show={@COUNT}&amp;skip={number(@SKIPTO) - number(@COUNT)}{$artspsearchphrase}&amp;contenttype={@CONTENTTYPE}&amp;ArticleSortby={@SORTBY}" xsl:use-attribute-sets="mARTICLESEARCH_link_previouspage">
			<xsl:copy-of select="$m_previouspageartsp"/></a></b>|</span>
	</xsl:template>
