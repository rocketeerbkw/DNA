...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
			...
			<br />
				...
				<div class="myspace-r">		
					...
					<xsl:element name="{$text.subheading}" use-attribute-sets="text.subheading">
						<strong class="white">my profile tools</strong>
					</xsl:element>
	...
	<xsl:otherwise>
		...
		<xsl:when test="MASTHEAD=1 and not(../ARTICLE-PREVIEW)">
			...
			<!-- SC --><!-- ADDED: condition that presents different text to editors -->
			<xsl:choose>
				<xsl:when test="$test_IsEditor">
					<xsl:call-template name="box.crumb">
						...
						<xsl:with-param name="box.crumb.value">my profile</xsl:with-param>
				...
				<xsl:otherwise>
					<xsl:call-template name="box.crumb">
						<xsl:with-param name="box.crumb.href" select="concat($root,'U',/H2G2/VIEWING-USER/USER/USERID)" />
						<xsl:with-param name="box.crumb.value">my profile</xsl:with-param>
						<xsl:with-param name="box.crumb.title">remove introduction</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="$test_IsEditor">
					<xsl:call-template name="box.step">
						<xsl:with-param name="box.step.text"><p>Use the boxes below to write an introduction about yourself. Don't include anything that you wouldn't want other members to see.</p></xsl:with-param>
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="box.step">
						<xsl:with-param name="box.step.title">This page allows you to remove your introduction.</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:when test="MASTHEAD=1">
			...
			<div class="mostrecent">
				<xsl:call-template name="box.crumb">
				 <xsl:with-param name="box.crumb.href" select="concat($root,'U',/H2G2/VIEWING-USER/USER/USERNAME)" />
					<xsl:with-param name="box.crumb.value">my profile</xsl:with-param>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:call-template name="box.crumb">
						<xsl:with-param name="box.crumb.href">#edit</xsl:with-param>
						<xsl:with-param name="box.crumb.value">remove introduction</xsl:with-param>
						<xsl:with-param name="box.crumb.title">preview</xsl:with-param>
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
			...
			<div><span class="textbase"><strong>step 2: preview introduction.</strong></span><br />
			<font size="2"><p>you are happy click "publish" otherwise edit your introduction using the boxes below and click "preview" again.</p></font>
			</div>
		...
		<!-- PREVIEW -->
		<!-- SC --><!-- REMOVED. No need for preview since you cannot change your profile -->
	...
	<!-- SC -->
	<!-- REMOVED for site closure CV 28/02/07 -->
	<!--
	...
	<table border="0" cellspacing="0" cellpadding="0" width="390">
		<tr><td>
	-->
	<!-- SC --><!-- ADDED: conditional. only editors can edit profile -->
	...
	<!-- SC --><!-- ADDED: hidden input to remove profile if user is NOT an editor -->
...
<!-- 		<xsl:apply-templates select="." mode="t_previewbutton"/>
 -->		
		...
		<!-- SC --><!-- Need to change button to 'Remove Intro'-->
	...
	<!-- SC --><!-- controls submit button on useredit page, need to change to 'Remove Intro' -->
