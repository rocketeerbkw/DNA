...
<xsl:template name="EDITOR_REVIEW">

	<!-- DEBUG -->
	<xsl:call-template name="TRACE">
	<xsl:with-param name="message">EDITOR_PAGE</xsl:with-param>
	<xsl:with-param name="pagename">formeditorreview.xsl</xsl:with-param>
	</xsl:call-template>
	<!-- DEBUG -->
<div class="form-wrapper">
   ...
   <!-- ARTICLE TYPE -->
	<table border="1" cellspacing="2" cellpadding="2">
		...
		<xsl:if test="$current_article_type=41 or $current_article_type=44 or $current_article_type=40">
		<tr>
		<td>
		<xsl:element name="{$text.base}" use-attribute-sets="text.base">
			<xsl:choose>
		...
		<xsl:if test="not($layout_type='layout_a')">
		<tr>	
		<td>
		<xsl:element name="{$text.base}" use-attribute-sets="text.base">
			<xsl:choose>
		...
		<td>
			<xsl:choose>
				...
				<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and $layout_type='layout_b'"><input type="text" name="PICTUREWIDTH" value="160" /></xsl:when>	
				<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and $layout_type='layout_a'"><input type="text" name="PICTUREWIDTH" value="408" /></xsl:when>			
				<xsl:when test="/H2G2/TYPED-ARTICLE-EDIT-FORM and $current_article_type=1">
		...
		<td>
			<xsl:choose>
			<!-- pre-populate the form GuideML -->
				<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and $layout_type='layout_b'"><input type="text" name="PICTUREHEIGHT" value="120" /></xsl:when>	
				<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and $layout_type='layout_a'"><input type="text" name="PICTUREHEIGHT" value="138" /></xsl:when>	
				<xsl:when test="/H2G2/TYPED-ARTICLE-EDIT-FORM and $current_article_type=1">
...
&lt;/SNIPPET&gt;</xsl:when>
	...
	<xsl:element name="{$text.base}" use-attribute-sets="text.base">
			...
			<font color="#FF0000">* <b><xsl:value-of select="$memberOrEditor" /> name:</b></font>
			</xsl:when>
			<xsl:otherwise>
			<b><xsl:value-of select="$memberOrEditor" /> name:</b>
			</xsl:otherwise>
			</xsl:choose>
	</xsl:element>
	</td>
	...
	<xsl:element name="{$text.base}" use-attribute-sets="text.base">
			...
			<font color="#FF0000">* <b><xsl:value-of select="$memberOrEditor" /> DNAID:</b></font>
			</xsl:when>
			<xsl:otherwise>
			<b><xsl:value-of select="$memberOrEditor" /> DNAID:</b>
			</xsl:otherwise>
			</xsl:choose>
	</xsl:element>
	</td>
	...
	<xsl:if test="not($article_type_group='interview' or $article_type_group='feature' or $article_type_group='column')">
	<tr><td>
	<xsl:element name="{$text.base}" use-attribute-sets="text.base">
	<b>rating:</b>
	</xsl:element>
	</td>
	<td><xsl:apply-templates select="." mode="r_ratingtype"/></td></tr>
	</xsl:if>
	...
	<xsl:choose>
	<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and /H2G2/ARTICLEINFO/NAME='talk'">Find out what our members have been up to. Pictures, music tracks, reviews and discussion.</xsl:when>
	<xsl:otherwise>
	...
	<textarea name="KEYWORDS" ID="KEYWORDS" cols="50" rows="5">
		<xsl:choose>
	<xsl:when test="@TYPE='TYPED-ARTICLE-CREATE' and /H2G2/ARTICLEINFO/NAME='talk'">community members, online community, cultural forum, discussion forum, discuss culture, talk</xsl:when>
	<xsl:otherwise>
	...
	<tr>
		...
		<xsl:choose>
			<xsl:when test="$current_article_type=40 or $current_article_type=41">
			<input type="radio" name="ARTICLEFORUMSTYLE" value="0" /> Conversations - FORUMTHREAD<br />
			<input type="radio" name="ARTICLEFORUMSTYLE" value="1" checked="checked" /> Comments - FORUMTHREADPOSTS
			</xsl:when>
			<xsl:otherwise>
...
&lt;LINK TYPE="logo" DNAID="A2283095"&gt;type link here&lt;/LINK&gt;</xsl:when>
	...
	<xsl:variable name="memberOrEditor">
	<xsl:choose>
		<xsl:when test="$article_type_user='member' and $article_type_group='feature'">
		member
		</xsl:when>
		<xsl:otherwise>
		editor
