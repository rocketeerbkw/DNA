...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
			...
			<tr>
				<td>
					<font xsl:use-attribute-sets="mainfont">
						<xsl:copy-of select="$m_NewUsersPageExplanatory"/><br /><br/>
						<a href="http://dnadev.national.core.bbc.co.uk/dna/filmnetwork/NewUsers?TimeUnits=36&amp;UnitType=month&amp;Filter=off&amp;thissite=1&amp;show=10000">
							<strong>Or click here to show all members who have joined in the past 36 months, on one page</strong>
						</a>
	...
	-->
	<xsl:template match="USER-LIST" mode="newusers_userlist">
		<script src="http://www.bbc.co.uk/filmnetwork/includes/sorttable.js"></script>
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="sortable" id="newuserstable">
			<tr align="left">
				<th>First Names</th>
				<th>Last Name</th>
				<th>User Name</th>
		...
		<br/>
		<xsl:variable name="numberofusers" select="count(/H2G2/NEWUSERS-LISTING/USER-LIST/USER)"/>
		<p>Number of users on this page: <xsl:value-of select="$numberofusers"/></p>
	...
	<tr valign="top">
    		<td>
				...
				<xsl:apply-templates select="." mode="c_postingflag"/>
        				<xsl:apply-templates select="FIRSTNAMES" mode="t_newusers"/>
    			</font>
    		</td>
    		<td>
    			<font xsl:use-attribute-sets="mainfont">
    				<xsl:apply-templates select="LASTNAME" mode="t_newusers"/>
    			</font>
    		</td>
		<td>
			<font xsl:use-attribute-sets="mainfont">
		...
		<td>
			<font xsl:use-attribute-sets="mainfont">
				<xsl:value-of select="DATE-JOINED/DATE/@YEAR"/>
				<xsl:text> </xsl:text>
				<xsl:value-of select="DATE-JOINED/DATE/@MONTH"/>
				<xsl:text> </xsl:text>
				<xsl:value-of select="DATE-JOINED/DATE/@DAY"/>
			</font>
		</td>
    	</tr>
</xsl:template>
	
<!-- First names of newusers -->
<xsl:template match="FIRSTNAMES" mode="t_newusers">
...
<!-- Last names of newusers-->
<xsl:template match="LASTNAME" mode="t_newusers">
	<a xsl:use-attribute-sets="mUSERNAME_t_newusers">
		<xsl:attribute name="HREF">
			<xsl:value-of select="$root"/>U<xsl:value-of select="../USERID"/>
		</xsl:attribute>
