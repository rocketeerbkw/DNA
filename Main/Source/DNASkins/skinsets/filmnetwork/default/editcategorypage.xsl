...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<table width="635" border="0" cellspacing="0" cellpadding="0">
		  <!-- Spacer row -->
          <tr>
            <td><img src="/f/t.gif" width="635" height="1" class="tiny" alt="" /></td>
		  ...
		  <tr>
			...
			<img src="/f/t.gif" width="1" height="5" alt="" />
					 <div class="topboxcopy"><xsl:choose>
						<xsl:when test="count(HIERARCHYDETAILS/ANCESTRY/ANCESTOR) = 2">	<xsl:choose>
			<xsl:when test="/H2G2/PARAMS/PARAM[NAME = 's_title']/VALUE = 'add place'">
			Choose your place from the list below.							
							</xsl:when>
							<xsl:otherwise>
							You can add up to three specialisms. <br />To add a specialism, click on the relevant one and then confirm. <br />To add more specialisms you need to repeat this process. 
							</xsl:otherwise>
...
<table width="635" border="0" cellspacing="0" cellpadding="0">
		  ...
		  <tr>
			...
			<tr><td width="20"></td><td width="480">
<!-- LEFT COL MAIN CONTENT -->
	<xsl:if test="$test_IsEditor">
		<div class="textmedium">
			<xsl:apply-templates select="HIERARCHYDETAILS" mode="c_editcat"/><br />
		</div>
	</xsl:if>
	...
	<xsl:if test="$test_IsEditor">
		...
		<xsl:if test="$test_IsEditor or count(HIERARCHYDETAILS/ANCESTRY/ANCESTOR) = 3">	
	<xsl:apply-templates select="HIERARCHYDETAILS/TAGINFO" mode="c_tagnode"/><br/><br/></xsl:if>


	<xsl:apply-templates select="HIERARCHYDETAILS/MEMBERS" mode="c_editcat"/>
	<!-- count <xsl:value-of select="count(HIERARCHYDETAILS/ANCESTRY/ANCESTOR)" /> count -->
	

	<xsl:if test="$test_IsEditor"><xsl:apply-templates select="HIERARCHYDETAILS/TAGINFO" mode="c_returntotagitem"/></xsl:if>
<br /><br />
	<strong><a href="{$root}A{/H2G2/EDITCATEGORY/ACTIVENODE/@ACTIVEID}">back to my profile</a></strong>&nbsp;<img src="{$imagesource}furniture/myprofile/arrowdark.gif" width="4" height="7" alt="" />
	...
	Use: Presentation of a node's description
	 -->
	<xsl:template match="DESCRIPTION" mode="r_editcat">
	<xsl:apply-templates select="." mode="t_editcat"/><br/>
	</xsl:template>
	<!--
	...
	<strong><xsl:apply-templates select="NAME" mode="t_nodename_ec"/></strong>
	<!--  category box -->
	
		<xsl:if test="$test_IsEditor"><xsl:apply-templates select="NODECOUNT" mode="c_subject_ec"/></xsl:if>
		<xsl:if test="SUBNODES"><br /></xsl:if>
		<xsl:apply-templates select="SUBNODES/SUBNODE" mode="c_subnodename_ec"/>
		<br/>
		<xsl:if test="$test_IsEditor"><xsl:apply-templates select="." mode="c_movesubject"/>
		<xsl:apply-templates select="." mode="r_deletesubject"/><br /><br /></xsl:if>
	...
	<xsl:variable name="m_tagtothisnode"><strong>Confirm add me to <xsl:value-of select="/H2G2/EDITCATEGORY/HIERARCHYDETAILS/DISPLAYNAME" /></strong></xsl:variable>
	<!--
