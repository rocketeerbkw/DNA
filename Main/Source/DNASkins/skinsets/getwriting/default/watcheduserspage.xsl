...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:with-param name="message">WATCHED-USERS_MAINBODY<xsl:value-of select="$current_article_type" /></xsl:with-param>
	...
	<xsl:element name="{$text.base}" use-attribute-sets="text.base">
		<xsl:choose>
			<xsl:when test="@TYPE='delete'">
				<xsl:copy-of select="$m_deletedfollowingfriends"/><br/>
				<strong><a href="{USER/USERID}"><xsl:value-of select="USER/USERNAME"/></a></strong>
			</xsl:when>
