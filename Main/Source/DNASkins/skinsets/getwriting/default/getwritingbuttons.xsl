...
<xsl:variable name="button.editlarge"><img src="{$graphics}buttons/button_editlarge.gif" width="78" height="25" alt="edit" border="0" /></xsl:variable>
	...
	<xsl:variable name="m_acceptlink"><img src="{$graphics}buttons/button_accept.gif" width="76" height="20" alt="accept" border="0" /></xsl:variable>
	<xsl:variable name="m_rejectlink"><img src="{$graphics}buttons/button_decline.gif" width="76" height="20" alt="decline" border="0" /></xsl:variable>
	<xsl:variable name="button.admin"><img src="{$graphics}buttons/button_admin.gif" width="66" height="20" alt="admin" border="0" /></xsl:variable>

<xsl:variable name="button.retrieve"><img src="{$graphics}buttons/button_retrieve.gif" width="72" height="20" alt="retrieve" border="0" /></xsl:variable>


	<xsl:variable name="m_delete">
	<xsl:choose>
	<xsl:when test="/H2G2/@TYPE='TYPED-ARTICLE'"><img src="{$graphics}buttons/button_removelarge.gif" width="94" height="25" alt="remove" border="0"  vspace="5"/></xsl:when>
	<xsl:otherwise><xsl:copy-of select="$button.remove" /></xsl:otherwise>
</xsl:choose>
	
	
	</xsl:variable>
	 <xsl:variable name="m_removeme"><xsl:copy-of select="$button.remove" /></xsl:variable> 
	<xsl:variable name="m_editme">
	<!-- ################ removed for site pulldown ################ -->
	<xsl:choose>
	<xsl:when test="$test_IsEditor">
	<img src="{$graphics}buttons/button_edit.gif" width="66" height="22" alt="remove" border="0" />
	</xsl:when>
	<xsl:otherwise>
	<img src="{$graphics}buttons/button_remove.gif" width="72" height="20" alt="remove" border="0" />
	</xsl:otherwise>
	</xsl:choose>
	</xsl:variable>
...
<!-- 	<xsl:attribute-set name="form.publish" use-attribute-sets="form.default.properties">
	...
	<!-- added for removal on cancelation version of site changed from publish button above-->
	<xsl:attribute-set name="form.publish" use-attribute-sets="form.default.properties">
		<xsl:attribute name="src"><xsl:value-of select="$graphics" />buttons/button_remove.gif</xsl:attribute>
