...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-	microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	*******************************************************************************************
	...
	<xsl:template match="BOARDPROMOLIST" mode="choose_default">
		<form name="setdefaultpromo" method="post" action="{$root}boardpromos">
			...
			<div id="instructional">
				<p>The default promo will appear on all messageboard pages that have no other promos assigned to them.</p>
			</div>
			<div id="contentArea">
			...
			<div class="centralAreaRight">
				...
				<div class="footer">
					...
					<span class="buttonLeftA">	
						...
						<div class="buttonThreeD">
							<a href="{$root}boardpromos?promoid=0&amp;page=promoname&amp;s_promotype=tag">Add new promo &gt; &gt;</a>
		...
		<xsl:choose>
			<xsl:when test="count(preceding-sibling::BOARDPROMO) mod 2 = 0">
				<tr class="stripeOne">
					...
					<td class="dataStatus" style="text-align:left;">
						<a href="{$root}boardpromos?page=promoname&amp;promoid={ELEMENTID}&amp;s_promotype=tag" style="color:#000000;">
							Edit...
						</a>
					</td>
					<td>
			...
			<div id="contentArea">
				<div class="centralAreaRight">
					...
					<div class="centralArea">
						...
						<xsl:apply-templates select="/H2G2/PARAMS/PARAM[NAME='s_promotype'][VALUE='tag']" mode="promo"/>
				...
				<div class="centralAreaRight">
					...
					<div class="centralArea">
						...
						<!--<p>
							...
							<br/>
							(busiest conversations, editor’s pick, etc)
						</p>-->
						<p>
	...
	<xsl:template match="PARAM" mode="promo">
		<br/>
		<h3 class="adminFormHeader">Either,</h3> 
		<xsl:text> enter the tag(s) it will be used for</xsl:text>
		<br/>
		<input type="text" name="phrases">
			<xsl:attribute name="value">
				<xsl:for-each select="/H2G2/BOARDPROMOPAGE/BOARDPROMOLIST/BOARDPROMO[@EDITSTATUS = 'ACTIVE']/PHRASES/PHRASE">
					<xsl:value-of select="NAME"/>
		...
		<br/>
		<h3 class="adminFormHeader">Or,</h3>
		<xsl:text> choose the ‘editorial tag(s)’ it will be used for</xsl:text>
		<br/>
		<select name="phrase" size="1" >
			...
			<xsl:for-each select="/H2G2/THREADSEARCHPHRASE/SITEKEYPHRASES/KEYPHRASE[not(NAME='England' or NAME='Channel Islands')]">
				<option value="{TERM}">
					<xsl:choose>
						<xsl:when test="TERM = /H2G2/THREADSEARCHPHRASE/PHRASES/PHRASE/TERM">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:when>
						<!--xsl:when test="TERM = /H2G2/VIEWING-USER/USER/REGION and not(contains($list_of_regions, /H2G2/THREADSEARCHPHRASE/PHRASES/PHRASE/TERM))"-->
						<xsl:when test="(TERM = /H2G2/VIEWING-USER/USER/REGION) and not(/H2G2/THREADSEARCHPHRASE/PHRASES/PHRASE/NAME = msxsl:node-set($list_of_regions)/region)">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:when>
					</xsl:choose>
					<xsl:value-of select="NAME"/>
