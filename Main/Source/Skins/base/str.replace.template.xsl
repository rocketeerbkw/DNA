<?xml version="1.0"?>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:str="http://exslt.org/strings" xmlns:exsl="http://exslt.org/common" extension-element-prefixes="str exsl" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
			...
			<xsl:when test="function-available('msxsl:node-set')">
              ...
              if they are one of the other XPath types -->
				...
				<xsl:variable name="replacements-rtf">
					<xsl:for-each select="msxsl:node-set($search-nodes-rtf)/node()">
						<xsl:variable name="pos" select="position()"/>
						<replace search="{.}">
							<xsl:copy-of select="msxsl:node-set($replace-nodes-rtf)/node()[$pos]"/>
						</replace>
					</xsl:for-each>
				</xsl:variable>
				<xsl:variable name="sorted-replacements-rtf">
					<xsl:for-each select="msxsl:node-set($replacements-rtf)/replace">
				...
				<xsl:call-template name="str:_replace">
					...
					<xsl:with-param name="replacements" select="msxsl:node-set($sorted-replacements-rtf)/replace"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:message terminate="yes">
            ERROR: template implementation of str:replace relies on msxsl:node-set().
         </xsl:message>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
