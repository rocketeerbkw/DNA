    ...
    <xsl:choose>
      <xsl:when test="$mod='items'">
        <items xmlns="http://purl.org/rss/1.0/">
          <rdf:Seq>
            <xsl:apply-templates select="/H2G2/ARTICLESEARCH/ARTICLES/ARTICLE" mode="rdf_resource_articlesearchphrase"/>
          </rdf:Seq>
        </items>
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates select="/H2G2/ARTICLESEARCH/ARTICLES/ARTICLE" mode="rss1_articlesearchphrase"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
