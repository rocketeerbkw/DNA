...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
    ...
    <xsl:template match="VIEWING-USER" mode="library_memberservice_policyurl">
        <xsl:param name="ptrt" />
        
        <xsl:apply-templates select="/H2G2/SSOSERVICE | /H2G2/CURRENTSITESSOSERVICE" mode="library_sso_policyurl">
            <xsl:with-param name="ptrt" select="$ptrt" />            
        </xsl:apply-templates>
    </xsl:template>
    
    
    <xsl:template match="VIEWING-USER[/H2G2/SITE/IDENTITYSIGNIN = 1]" mode="library_memberservice_policyurl">
        <xsl:param name="ptrt" />
