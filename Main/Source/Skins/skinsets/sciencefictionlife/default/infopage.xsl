...
<!-- * column 2 -->
	...
	<xsl:if test="INFO/@MODE='articles' or INFO/@MODE='conversations'">
		<div class="rssModule">
		
		<xsl:variable name="infotype">
		<xsl:choose>
			<xsl:when test="INFO/@MODE='articles'">art</xsl:when>
			<xsl:when test="INFO/@MODE='conversations'">conv</xsl:when>
		</xsl:choose>	
		</xsl:variable>
		
		<xsl:element name="A">
		<xsl:attribute name="HREF"><xsl:value-of select="$root" />xml/info?cmd=<xsl:value-of select="$infotype" />&amp;s_xml=rss</xsl:attribute>
	...
	<!-- end rss feed link -->
		...
		<div class="morepost-h">
			...
			<xsl:choose>
				<xsl:when test="INFO/@MODE='articles'">
				 <!-- CREATE A NEW REVIEW -->
							<div class="myspace-u">
						<div class="myspace-r">	
