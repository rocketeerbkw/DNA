...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
	...
	<xsl:template name="m_posthasbeenpremoderated">
		<p>Thank you for adding a comment to British Film.<br/>
		All contributions by new British Film members are pre-moderated. This means that your first few articles or comments will not appear on the site immediately, as they need to be checked by a moderator first. After you have used the British Film for a while, you will become a trusted user and your contibutions will appear as soon as you post them.
	...
	<xsl:template name="m_posthasbeenautopremoderated">
		<p>Thank you for adding a comment to British Film.<br/>
		All contributions by new British Film members are pre-moderated.  This means that your first few comments will not appear on the site immediately, as they need to be checked by a moderator first.   After you have used the British Film for a while, you will become a trusted user and your comments will appear on the site as soon as you post them.<br/>
		The BBC receives thousands of messages every day so please be patient.</p>
...
Welcome. You're now registered to British Film.<br/>
 </xsl:template>
...
<xsl:template name="m_articlehiddentext">
	...
	<xsl:choose>
		<xsl:when test="$ownerisviewer = 1">
			...
			<p><a href="http://dnadev.national.core.bbc.co.uk/dna/britishfilm/TypedArticle?aedit=new&amp;type={$current_article_type}&amp;h2g2id={/H2G2/ARTICLE/ARTICLEINFO/H2G2ID}"><img src="{$imagesource}edit_{$article_type_group}.gif" width="118" height="23" alt="edit this {$article_type_group}"/></a></p>
		</xsl:when>
		<xsl:otherwise>
			...
			<p xsl:use-attribute-sets="ArticleText">This page is not currently available. Please click here to return to the
<a href="{$root}">homepage</a>.</p>
		</xsl:otherwise>
	</xsl:choose>
	</div>
</xsl:template>
...
<xsl:template name="m_articledeletedbody">
	<div class="bodytext">
		<p>This <xsl:value-of select="$m_article"/> has been removed by a moderator because it broke the <a href="http://www.bbc.co.uk/dna/britishfilm/houserules">house rules</a> in some way.</p>
	...
	<xsl:template name="m_cantpostnotregistered">
		...
		<P>If you haven't already registered with us as <xsl:value-of select="$m_usera"/>
					<xsl:value-of select="$m_user"/>, please <A xsl:use-attribute-sets="nm_cantpostnotregistered2" href="{$sso_nopostregisterlink}">click here to register</A>.</P>
...
terms of use. <A xsl:use-attribute-sets="nm_cantpostnoterms1" href="{$root}LoginTerms?pa=postforum&amp;pt=forum&amp;forum={@FORUMID}&amp;pt=thread&amp;thread={@THREADID}&amp;pt=post&amp;post={@POSTID}">Click here to complete your login</A>.</P>
