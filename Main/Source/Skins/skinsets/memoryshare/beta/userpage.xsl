...
<xsl:stylesheet version="1.0"
	...
	Use: sets the number of recent conversations and articles to display
	 -->
	<xsl:variable name="postlimitentries" select="9"/>
	<xsl:variable name="articlelimitentries" select="9"/>
	<xsl:variable name="clublimitentries" select="9"/>
	<xsl:variable name="morearticlesshow" select="9"/>
			...
			<xsl:when test="/H2G2/PARAMS/PARAM[NAME = 's_display']/VALUE = 'register'">
				...
				<div id="ms-std">
					...
					<div id="ms-std-content">
						<p>
							Welcome, this is the first time that you have come to the site - please take a moment to <a href="{$root}userdetails">set&nbsp;your&nbsp;preferences</a> before you continue.
						</p>
					</div>
	...
	<xsl:template match="H2G2" mode="r_displayuserpage">
		...
		<div id="ms-list-timeline">
			<div id="ms-list-timeline-header"><xsl:comment> ms-list-timeline-header </xsl:comment></div>
		...
		<div id="ms-post-timline">
			...
			<xsl:if test="$test_IsAdminUser">
				<div class="editbox">
					...
					<a href="{$root}inspectuser?userid={/H2G2/ARTICLE/ARTICLEINFO/PAGEAUTHOR/EDITOR/USER/USERID}">
						inspectuser: U<xsl:value-of select="/H2G2/ARTICLE/ARTICLEINFO/PAGEAUTHOR/EDITOR/USER/USERID"/>
					</a>
					<br/>
					<a href="{$root}moderationhistory?h2g2id={/H2G2/ARTICLE/ARTICLEINFO/H2G2ID}">
						moderation history: A<xsl:value-of select="/H2G2/ARTICLE/ARTICLEINFO/H2G2ID"/>
					</a>
					<br />
