...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="ARTICLE" mode="c_articlepage">
		<xsl:choose>
			...
			<xsl:when test="ARTICLEINFO/STATUS[@TYPE='7']">
				<xsl:call-template name="SimpleOutputMessage">
			...
			<xsl:when test="GUIDE/BODY/INVALIDARTICLE">
				<xsl:call-template name="SimpleOutputMessage">
					<xsl:with-param name="message">
						<xsl:text>1</xsl:text>
					</xsl:with-param>
	...
	<xsl:template name="SimpleOutputMessage">
			...
			<div id="ms-std-content">
				<xsl:choose>
					<xsl:when test="$message = '1'">
						<p>Sorry, we couldn't find the memory you were looking for.<br/><br/><a href="{$articlesearchbase}">Try a search</a></p>
					</xsl:when>
					<xsl:when test="$message = '2'">
						<p>Sorry, this memory has been removed by the author or by a moderator.<br/><br/><a href="{$articlesearchbase}">Try a search</a></p>
					</xsl:when>					
				</xsl:choose>
			</div>
	...
	<xsl:template match="ARTICLE" mode="decadeSearch">
		<xsl:variable name="decade">
			<xsl:if test="DATERANGESTART and DATERANGEEND">
		...
		<xsl:if test="$decade and $decade != 'null' and $decade != ''">
			<xsl:value-of select="$root"/>
			<xsl:value-of disable-output-escaping="yes" select="$articlesearchbase"/>
			<xsl:text disable-output-escaping="yes">&amp;datesearchtype=2</xsl:text>
			<xsl:text disable-output-escaping="yes">&amp;startday=1</xsl:text>
			<xsl:text disable-output-escaping="yes">&amp;startmonth=1</xsl:text>
			<xsl:text disable-output-escaping="yes">&amp;startyear=</xsl:text>
			<xsl:value-of select="$decade"/>
			<xsl:text disable-output-escaping="yes">&amp;endday=31</xsl:text>
			<xsl:text disable-output-escaping="yes">&amp;endmonth=12</xsl:text>
			<xsl:text disable-output-escaping="yes">&amp;endyear=</xsl:text>
			<xsl:value-of select="$decade+9"/>
			<xsl:text disable-output-escaping="yes">&amp;s_dateview=decade</xsl:text>
		</xsl:if>
	</xsl:template>
