...
<xsl:stylesheet version="1.0" xmlns:tom="http://purl.org/rss/1.0/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dcterms="http://purl.org/dc/terms/" exclude-result-prefixes="msxsl local s dt tom">
		...
		<xsl:choose>
			<xsl:when test="$mod='items'">
				<items xmlns="http://purl.org/rss/1.0/">
					<rdf:Seq>
						<xsl:apply-templates select="/H2G2/SEARCH/SEARCHRESULTS/ARTICLERESULT[SITEID=$site_number]" mode="rdf_resource_articlesearch"/>
					</rdf:Seq>
				</items>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="/H2G2/SEARCH/SEARCHRESULTS/ARTICLERESULT[SITEID=$site_number]" mode="rss1_articlesearch"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template name="SEARCH_RSS2">
		<xsl:apply-templates select="/H2G2/SEARCH/SEARCHRESULTS/ARTICLERESULT[SITEID=$site_number]" mode="rss2_articlesearch"/>
	</xsl:template>
	
	<!-- 
	
	-->
	<xsl:template match="ARTICLERESULT" mode="rdf_resource_articlesearch">
		<rdf:li rdf:resource="{$thisserver}{$root}A{H2G2ID}"/>
	</xsl:template>
	<!-- 
	
	-->
	<xsl:template match="ARTICLERESULT" mode="rss1_articlesearch">
	...
	<xsl:template match="ARTICLERESULT" mode="rss2_articlesearch">
		<item>
			<title>
				<xsl:value-of select="SUBJECT"/>
			</title>
