...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
    ...
    <xsl:template match="POST" mode="object_post_generic">
        ...
        <xsl:variable name="firstPostId" select="../POST[@INDEX = 0]/POSTID"/>
        ...
        <li id="p{@POSTID}">
            ...
            <xsl:choose>
                ...
                <xsl:when test="@HIDDEN = 2 or @HIDDEN = 6">
                    ...
                    <p class="dna-boards-failedpost">
                        This post has been temporarily hidden, because a moderator has referred it to a supervisor, BBC host or the Central Communities Team for a decision as to whether it contravenes the <a href="{$houserulespopupurl}" class="popup">House Rules</a> in some way. A decision will be made as quickly as possible..
                    </p>
                </xsl:when>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <p class="dna-boards-inreplyto">
                        	<a href="/ouch/messageboards/AddThread?inreplyto={@POSTID}" class="id-cta">
                                <xsl:call-template name="library_memberservice_require">
                                    <xsl:with-param name="ptrt">
                                    	<xsl:text>/ouch/messageboards/AddThread?inreplyto=</xsl:text>
            ...
            <xsl:apply-templates select="USER" mode="library_userstate_editor">
                <xsl:with-param name="false">
                    <p class="flag">
                        <a class="popup complain">
                            <xsl:attribute name="href">
                                <xsl:value-of select="concat('UserComplaintPage?PostID=' , @POSTID, '&amp;s_start=1&amp;s_ptrt=')" />
            ...
            <xsl:if test="@INREPLYTO">
              <p class="dna-boards-thisreplyto">
                This is a reply to <a href="{concat('F', parent::FORUMTHREADPOSTS/@FORUMID, '?thread=', parent::FORUMTHREADPOSTS/@THREADID, '#p', @INREPLYTO)}">this message</a>.
              </p>
            </xsl:if>
