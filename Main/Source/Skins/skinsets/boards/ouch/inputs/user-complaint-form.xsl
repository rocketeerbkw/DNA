...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:doc="http://www.bbc.co.uk/dna/documentation"  exclude-result-prefixes="doc">
    ...
    <xsl:template match="USER-COMPLAINT-FORM[/H2G2/PARAMS/PARAM[NAME = 's_start'][VALUE = 1]] | USERCOMPLAINT[/H2G2/PARAMS/PARAM[NAME = 's_start'][VALUE = 1]]" mode="input_user-complaint-form">
        <div class="content">
            ...
            <p class="action">
                <xsl:choose>
                	<xsl:when test="POST-ID or @POSTID">
                		<a href="?PostId={(POST-ID | @POSTID)[1]}&amp;s_ptrt={/H2G2/PARAMS/PARAM[NAME = 's_ptrt']/VALUE}&amp;s_start=2">Register my Complaint</a>
                	</xsl:when>
                	<xsl:otherwise>
                		<a href="?h2g2ID={(H2G2-ID | @H2G2ID)[1]}&amp;s_ptrt={/H2G2/PARAMS/PARAM[NAME = 's_ptrt']/VALUE}&amp;s_start=2">Register my Complaint</a>
                	</xsl:otherwise>
                </xsl:choose>
                          ...
                          <xsl:attribute name="href">
                          	<xsl:apply-templates select="/H2G2/VIEWING-USER" mode="library_memberservice_loginurl">
                          	  <xsl:with-param name="ptrt">
                          	  		<xsl:choose>
		                          	  	<xsl:when test="POST-ID or @POSTID">
		                          	  		<xsl:value-of select="concat('UserComplaintPage?PostID=', (POST-ID | @POSTID)[1], '&amp;s_start=2')"/>
		                          	  	</xsl:when>
                          	  			<xsl:otherwise>
                          	  				<xsl:value-of select="concat('UserComplaintPage?h2g2ID=', (H2G2-ID | @H2G2ID)[1], '&amp;s_start=2')"/>
                          	  			</xsl:otherwise>
                          	  		</xsl:choose>
                          	  	</xsl:with-param>
                          	</xsl:apply-templates>
        ...
        <form action="UserComplaintPage" method="post" id="complaintForm"> 
            ...
            <div class="content">
            	<xsl:choose>
            		...
            		<xsl:otherwise>
            			<input type="hidden" value="{(H2G2-ID | @H2G2ID)[1]}" name="h2g2ID"/>
            		</xsl:otherwise>
            	</xsl:choose>
            	...
            	<p>Please choose which of the <a href="{$houserulespopupurl}">House Rules</a> you believe this post has broken. If you feel it breaks more than one rule, please choose the most serious breach.</p>
            </div>
            
            <div class="content">
                <h2>Reason for your complaint</h2>
                
                <p>I believe this post may break one of the <a href="{$houserulespopupurl}">House Rules</a> because it:</p>
                <p class="options">
                	<p class="options">
    ...
    <xsl:template match="USER-COMPLAINT-FORM | USERCOMPLAINT" mode="input_user-complaint-form">
        <form id="UserComplaintForm" action="UserComplaintPage" method="post"> 
           <div class="content"> 
           	<p>Please fill in the box below to tell us the reason you feel the post breaks this rule. When you are finished, click on Send Complaint so that it can be reviewed by a moderator.</p>
               <p>
                ...
                <p class="options">
                    <textarea id="reason" rows="10" cols="40" name="complainttext" class="textarea">
                    	<xsl:if test="/H2G2/PARAMS/PARAM[NAME = 's_complaintText']/VALUE and /H2G2/PARAMS/PARAM[NAME = 's_complaintText']/VALUE != 'Other'">
                    		...
                    		<xsl:apply-templates select="/H2G2/PARAMS/PARAM[NAME = 's_complaintText']/VALUE" mode="library_string_stringtolower"/>
    ...
    <xsl:template match="USER-COMPLAINT-FORM[MESSAGE/@TYPE = 'SUBMIT-SUCCESSFUL'] | USERCOMPLAINT[@MODID]" mode="input_user-complaint-form">
        <div class="content">
            ...
            <p>Your Moderation Reference ID is: <strong><xsl:value-of select="(MODERATION-REFERENCE | @MODID)[1]"/></strong></p>
