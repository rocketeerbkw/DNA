...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:local="#local-functions" xmlns:s="urn:schemas-	microsoft-com:xml-data" xmlns:dt="urn:schemas-microsoft-com:datatypes" exclude-result-prefixes="msxsl local s dt">
		...
		<div class="additionsec">
			<xsl:if test="$enable_rss='Y'">
				<div class="rssbox">	
					...
					<p class="feed">
						<xsl:call-template name="rss_info_link"/>
					</p>
	...
	<xsl:template name="rss_info_link">
		<xsl:variable name="raw_href">
			<xsl:value-of select="$root"/>
			<xsl:text>xml/info?s_xml=rss&amp;cmd=art&amp;show=15</xsl:text>
		</xsl:variable>
		<a>
			<!-- strip whitespace from href -->
			<xsl:attribute name="href">
				<xsl:value-of select="$raw_href"/>
			</xsl:attribute>
			<img src="{$imagesource}feed.gif" width="16" height="16" alt="RSS"/>
		</a>
		<xsl:text> </xsl:text>
		<a>
			<!-- strip whitespace from href -->
