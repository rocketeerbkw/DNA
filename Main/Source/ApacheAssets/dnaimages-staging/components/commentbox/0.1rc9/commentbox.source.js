		...
		function wrapText (nodeList, matchLength) {
			...
			if (glow.env.gecko) {
				...
				$(nodeList).each(function(i) {
				    ...
				    var matches = pattern.exec(this.innerHTML);
					...
					if (matches) {
					    ...
					    for (var i=0, l=matches.length; i < l; i++) {
					        ...
					        if (matches[i].length >= matchLength) {
					            ...
					            this.innerHTML = this.innerHTML.replace(matches[i], replace);
					        }
					    }
					}
				});
...
dna.comments = function (){
		...
		function AddEvents(f) {
				...
				if (formEl[0]) {
					...
					formEl[0].action = hack.replace(/vanilla/, "vanilla-json");
