var dna = function () {
	...
	var comments = {
		...
		preview: {
			...
			generate: function() {
				...
				var month = s.toLocaleDateString().toString().split(' ')[1];
				
				var d = {
					text:  utils.newlineToBr(comments.textEl[0].value),
					notables: ($('.dna-commentbox-userstate').slice(0, 1).get('a').slice(0, 1).hasClass('notables') ? 'class="notables"' : ''),
					name: $('.dna-commentbox-userstate').slice(0, 1).get('a').slice(0, 1).text(),
					count: comments.previewAreaEl.prev().prev().prev().get("li").length + 1,
					time: s.getHours() + ':' + s.getMinutes() + ' ' + ((s.getHours() < 12) ? 'am' : 'pm'),
					date: s.getDate() + ' ' + month.substring(0, 3) + ' ' + s.getFullYear()					
				};
				
				var t = '<li {notables}><span class="comment-number">{count}. </span><cite>At <a href="#postcomment" class="time">{time}</a> on <span class="date">{date}</span>, <span class="vcard"><span class="fn"><a href="#">{name}</a></span></span> wrote:</cite><p class="comment-text">{text}</p><p class="flag"><a href="#">Complain about this comment</a></p></li>';
	...
	return {
		...
		comments: {
			apply: function (targetForm, timeBetweenComments) {
				
				comments.defaultCountdown = timeBetweenComments || 120;
								
				$('.popup').each(function() {
					glow.events.addListener(this, 'click', function() {
						dna.utils.popup(this.href, 640, 440);
						return false;
					});
				});
				
				comments.AddEvents(targetForm);
...
glow.ready( function() {
	...
	/*
	 * Callback example
	dna.comments.post.onSuccess = function () {
		//do stuff..
	}
	 */
	
	dna.comments.apply('div.comments form', 12);
