/*********************************************************************************
	...
	create procedure getmediaassetswithkeyphrases @keyphraselist VARCHAR(8000),
	...
	Outputs:	Media Asset dataset
	Purpose:	Returns the Media Asset dataset with the given key phrases
	
*********************************************************************************/
CREATE PROCEDURE getmediaassetswithkeyphrases @keyphraselist VARCHAR(8000), @siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null
AS
BEGIN	
...
--Get total media assets for site with key phrases.
declare @total int
SELECT @total = COUNT(*) FROM (SELECT MA.ID FROM dbo.udf_splitvarchar(@keyphraselist) as s
