/*********************************************************************************
	...
	create procedure getmediaassetsforsite @siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null

	Author:		Steven Francis - from Martins SP
	Created:	08/11/2005
	Inputs:		@siteid int, @firstindex int, @lastindex int, @sortbycaption bit = null
	Outputs:	All Media Assets for the site with or without a given Phrase
	Purpose:	Returns the Media Assets for a site
...
--Get total media assets for site.
declare @total int
SELECT @total =  COUNT(*) FROM MediaAsset MA 
