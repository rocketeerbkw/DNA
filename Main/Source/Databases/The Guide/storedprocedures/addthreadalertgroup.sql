CREATE PROCEDURE addthreadalertgroup @threadid int, @userid int, @siteid int, @groupid int OUTPUT, @result int OUTPUT
--CREATE PROCEDURE addthreadalertgroup @threadid int, @userid int, @notifytype int, @siteid int, @listtype int, @groupid int OUTPUT, @result int OUTPUT
AS
-- Initialise the output parameter
SET @groupid = 0

-- Check to see if we already have this item 
DECLARE @ThreadType int
EXEC SetItemTypeValInternal 'IT_THREAD', @ThreadType OUTPUT
SELECT @GroupID = GroupID FROM dbo.AlertGroups WHERE ItemID = @ThreadID AND ItemType = @ThreadType AND SiteID = @SiteID AND UserID = @UserID
IF ( @GroupID > 0 )
BEGIN
...
DECLARE @Error int
INSERT INTO dbo.AlertGroups (UserID, ItemID, ItemType, SiteID, IsOwner) VALUES (@UserID, @ThreadID, @ThreadType, @SiteID, @IsOwner)
...
EXEC @Error = additemtoemailalertlistinternal @ListID, 'IT_THREAD', @ThreadID, @NotifyType, @GroupID, @Result OUTPUT
