/*********************************************************************************
	...
	create procedure getkeyphrasearticleaudioassethotlistwithphrase @keyphraselist VARCHAR(8000), @siteid int, @firstindex int, @lastindex int as
	...
	Outputs:	
	Purpose:	Get an article with audio asset tag cloud based on the provided site and phrases filter.
	
*********************************************************************************/
Create Procedure getkeyphrasearticleaudioassethotlistwithphrase @siteid int, @keyphraselist VARCHAR(500), @skip int = 0, @show int = 500, @sortbyphrase bit = null
AS
...
INNER JOIN dbo.AudioAsset aud WITH(NOLOCK) ON aud.MediaAssetID = AMA.MediaAssetID
...
INNER JOIN KeyPhrases kp2 WITH(NOLOCK) ON pns2.PhraseId = kp2.PhraseId
...
( SELECT kp.PhraseId,
	...
	INNER JOIN KeyPhrases kp WITH(NOLOCK) ON pns.PhraseId = kp.PhraseId
